...
import {
  ...
  verifyProofContract,
} from "../services/proof-verifier-service";

const router = Router();

const replayCache = new Map<string, number>();
const REPLAY_WINDOW_MS = Number(
  process.env.PROOF_REPLAY_WINDOW_MS || 10 * 60 * 1000,
);
const MAX_METADATA_BYTES = Number(
  process.env.PROOF_METADATA_MAX_BYTES || 128 * 1024,
);
...
function proofAuth(req: any, res: any, next: any): void {
  ...
  if (!authHeader || !authHeader.startsWith("Bearer ")) {
    res
      .status(401)
      .json({ code: "PROOF_AUTH_REQUIRED", error: "Authentication required" });
    return;
  }
  ...
  if (!user) {
    res
      .status(401)
      .json({ code: "PROOF_AUTH_REQUIRED", error: "Invalid or expired token" });
    return;
  }
...
function ensureRecruiter(req: any, res: any): boolean {
  ...
  if (req.user.role !== "recruiter" && req.user.role !== "admin") {
    res
      .status(403)
      .json({ code: "PROOF_FORBIDDEN", error: "Recruiter role required" });
    return false;
  }
  return true;
}
...
function isDid(value: unknown): boolean {
  return (
    typeof value === "string" &&
    /^did:[a-z0-9]+:[A-Za-z0-9._:%-]+$/i.test(value)
  );
}
...
router.post("/v1/proofs/metadata", proofAuth, (req, res) => {
  ...
  const {
    ...
    canonicalization: canonicalizationRaw,
  } = req.body || {};
  ...
  if (bytes > MAX_METADATA_BYTES) {
    return res
      .status(413)
      .json({
        code: "PROOF_METADATA_TOO_LARGE",
        error: "credential payload too large",
      });
  }
...
router.post("/v1/proofs/verify", proofAuth, async (req, res) => {
  ...
  if (payload.expected_issuer_did && !isDid(payload.expected_issuer_did)) {
    return res
      ...
      .json({
        ...
        error: "expected_issuer_did must be a valid DID",
      });
  }
  if (payload.expected_subject_did && !isDid(payload.expected_subject_did)) {
    return res
      ...
      .json({
        code: "PROOF_INPUT_INVALID",
        error: "expected_subject_did must be a valid DID",
      });
  }
  ...
  if (replayCache.has(replayHash)) {
    return res
      ...
      .json({
        ...
        valid: false,
        reason_codes: ["PROOF_REPLAY_DETECTED"],
      });
  }
