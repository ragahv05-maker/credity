...
const APP_NAME = process.env.APP_NAME || "credverse-recruiter";
...
export function initSentry(appName?: string): void {
  ...
  console.log(
    `[Sentry] Error monitoring initialized for ${appName || APP_NAME}`,
  );
}
...
export function captureException(
  ...
  Sentry.withScope((scope) => {
    const safeContext = sanitizeContext(context || {}) as Record<
      string,
      unknown
    >;
