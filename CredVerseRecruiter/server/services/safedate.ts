...
export function computeSafeDateBreakdown(
  input: SafeDateInput,
): SafeDateBreakdown {
  return {
    profile_integrity: Math.round(
      SAFEDATE_WEIGHTS.profile_integrity * clampRatio(input.profile_integrity),
    ),
    identity_confidence: Math.round(
      SAFEDATE_WEIGHTS.identity_confidence *
        clampRatio(input.identity_confidence),
    ),
    social_consistency: Math.round(
      SAFEDATE_WEIGHTS.social_consistency *
        clampRatio(input.social_consistency),
    ),
    behavior_stability: Math.round(
      SAFEDATE_WEIGHTS.behavior_stability *
        clampRatio(input.behavior_stability),
    ),
    risk_checks: Math.round(
      SAFEDATE_WEIGHTS.risk_checks * clampRatio(input.risk_checks),
    ),
  };
}

export function computeSafeDateScore(input: SafeDateInput): {
  score: number;
  factors: SafeDateBreakdown;
} {
...
export function normalizeSafeDateReasonCodes(
  reasonCodes: unknown,
): SafeDateReasonCode[] {
  if (!Array.isArray(reasonCodes)) return [];
  return reasonCodes.filter(
    (code): code is SafeDateReasonCode =>
      typeof code === "string" &&
      SAFEDATE_REASON_CODES.includes(code as SafeDateReasonCode),
  );
}

export function evaluateSafeDate(
  payload: SafeDateEvaluationRequestContract,
): SafeDateEvaluation {
  ...
  const signals_checked = Array.isArray(payload.evidence?.signals_checked)
    ? payload.evidence.signals_checked.filter(
        (item): item is string => typeof item === "string",
      )
    : [];

  const checks_run = Array.isArray(payload.evidence?.checks_run)
    ? payload.evidence.checks_run.filter(
        (item): item is string => typeof item === "string",
      )
    : [];
