...
type PersistedVerificationResult = Omit<VerificationResult, "timestamp"> & {
  timestamp: string;
};
type PersistedBulkVerificationResult = Omit<
...
function deserializeVerificationResult(
...
function serializeBulkResult(
  result: BulkVerificationResult,
): PersistedBulkVerificationResult {
  return {
    ...result,
...
function deserializeBulkResult(
  result: PersistedBulkVerificationResult,
): BulkVerificationResult {
  return {
    ...result,
    ...
    hydrationPromise = (async () => {
      ...
      for (const [verificationId, result] of loaded?.verificationCache || []) {
        verificationCache.set(
          verificationId,
          deserializeVerificationResult(result),
        );
      }
  ...
  persistChain = persistChain
    .then(async () => {
      await stateStore.save({
        verificationCache: Array.from(verificationCache.entries()).map(
...
export class VerificationEngine {
  ...
  constructor() {
    this.walletEndpoint =
      process.env.WALLET_ENDPOINT || "http://localhost:5002";
    ...
    try {
      ...
      return {
        ...
        __jwtMeta: {
          ...
          kid: header?.kid,
          hasSignature:
            typeof signatureSegment === "string" && signatureSegment.length > 0,
          rawJwt: jwt,
        },
      };
    } catch {
  ...
  private async verifySignature(credential: any): Promise<VerificationCheck> {
    ...
    const hasProof = Boolean(
      credential.proof || credential.signature || hasJwtSignature,
    );
    const proofType = hasJwtSignature
      ? "jwt"
      : (credential.proof?.type ?? "vc_proof");
  ...
  private async verifyIssuer(credential: any): Promise<VerificationCheck> {
    ...
    let issuerInfo =
      this.issuerRegistry.get(issuerDid.toLowerCase()) ||
      this.issuerRegistry.get(issuerDid);
    ...
    if (!issuerInfo) {
      try {
        ...
        const registryUrl =
          process.env.ISSUER_REGISTRY_URL || "http://localhost:5001";
        ...
        if (res.ok) {
          ...
          issuerInfo = {
            ...
            type: "academic",
            trustLevel:
              remoteIssuer.trustStatus === "trusted" ? "high" : "medium",
          ...
          console.warn(
            `[Verification] Issuer lookup failed for ${issuerDid}: ${res.status}`,
          );
        }
      } catch (e) {
        console.error(
          `[Verification] Failed to resolve issuer ${issuerDid} remotely:`,
          e,
        );
      }
    }
      ...
      for (const url of candidates) {
        ...
        if (res.status === 404) {
          return {
            ...
            details: {
              code: "ISSUER_CREDENTIAL_NOT_FOUND",
              revoked: true,
              checkedAt,
              source: url,
            },
   ...
   * Get bulk job result
   */
  async getBulkJobResult(
    id: string,
  ): Promise<BulkVerificationResult | undefined> {
    await ensureHydrated();
