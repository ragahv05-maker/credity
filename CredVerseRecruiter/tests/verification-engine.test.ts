...
vi.mock("@credverse/shared-auth", async () => {
  ...
  return {
    ...
    PostgresStateStore: class {
      constructor() {}
      load() {
        return Promise.resolve({ verificationCache: [], bulkJobs: [] });
      }
      save() {
  ...
  describe("verifyCredential", () => {
    const validCredential = {
      ...
      proof: {
        ...
        verificationMethod:
          "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnn3Zua2F72#z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnn3Zua2F72",
    ...
    it("successfully verifies a valid raw credential", async () => {
      ...
      const meaningfulChecks = result.checks.filter(
        (c) => c.status !== "skipped",
      );
    ...
    it("successfully parses and verifies a JWT credential", async () => {
      ...
      const body = Buffer.from(JSON.stringify(validCredential)).toString(
        "base64url",
      );
      ...
      expect(result.checks.find((c) => c.name === "JWT Format")?.status).toBe(
        "passed",
      );
      expect(result.status).toBe("verified");
    });
    ...
    it("handles signature validation failure", async () => {
      ...
      expect(
        result.checks.find((c) => c.name === "Signature Validation")?.status,
      ).toBe("failed");
    });
    ...
    it("handles expired credential", async () => {
      ...
      expect(
        result.checks.find((c) => c.name === "Expiration Check")?.status,
      ).toBe("failed");
    });
    ...
    it("handles revoked credential", async () => {
      fetchMock.mockImplementation(async (url) => {
        if (
          typeof url === "string" &&
          (url.includes("/status") || url.includes("/verify/"))
        ) {
          return {
            ok: true,
            status: 200,
      ...
      expect(
        result.checks.find((c) => c.name === "Revocation Check")?.status,
      ).toBe("failed");
    });
    ...
    it("handles missing blockchain anchor", async () => {
      ...
      expect(
        result.checks.find((c) => c.name === "Blockchain Anchor")?.status,
      ).toBe("warning");
    });
      ...
      const creds: CredentialPayload[] = [
        {
          raw: {
            ...
            proof: {
              type: "test",
              verificationMethod:
                "did:key:z6MkhaXgBZDvotDkL5257faiztiGiC2QtKLGpbnn3Zua2F72#key",
            },
          },
        },
        { raw: {} }, // Invalid
