...
describeSepolia("sepolia smoke: issue -> claim -> verify", () => {
  ...
  beforeAll(async () => {
    if (!deployerKey) {
      throw new Error(
        "Missing RELAYER_PRIVATE_KEY (or SEPOLIA_SMOKE_RELAYER_PRIVATE_KEY) for Sepolia smoke test",
      );
    }
    ...
    walletToken = sharedAuth.generateAccessToken({
      id: 1,
      username: "holder-smoke",
      role: "holder",
    });
    verifierToken = recruiterAuth.generateAccessToken({
      id: "verifier-smoke",
  ...
  it("anchors on-chain and verifies proof path", async () => {
    ...
    const txHash =
      typeof rawTxHash === "string"
        ? rawTxHash
        : rawTxHash?.hash || rawTxHash?.txHash || "";
