...
export default function ClaimsDashboard() {
    ...
    useQuery<ClaimsResponse>({
      ...
      queryFn: async () => {
        const res = await fetch(
          `/api/claims?period=${selectedPeriod}&limit=100`,
        );
        if (!res.ok) {
  ...
  const investigatePercent =
    stats.total > 0 ? Math.round((stats.investigate / stats.total) * 100) : 0;
  const rejectedPercent =
    stats.total > 0 ? Math.round((stats.rejected / stats.total) * 100) : 0;

  const amountSaved = useMemo(
    () => (stats.rejected + stats.investigate) * 15000,
    [stats],
  );
        ...
        <div className="flex items-center justify-between gap-3">
          <div>
            <p className="text-muted-foreground">
              Claims analytics and adjudication overview
            </p>
          </div>
          ...
          <Card>
            ...
            <CardContent>
              <p className="text-2xl font-bold text-amber-600">
                {isLoading ? "…" : stats.rejected + stats.investigate}
              </p>
              <p className="text-xs text-muted-foreground">
                Review + reject volume
              </p>
            </CardContent>
          </Card>
          <Card>
            ...
            <CardContent>
              <p className="text-2xl font-bold">
                ₹{(amountSaved / 100000).toFixed(1)}L
              </p>
              <p className="text-xs text-muted-foreground">
                Based on prevented fraud loss
              </p>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Distribution across automated outcomes
              </CardDescription>
            </CardHeader>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p className="text-xs mt-1">
                  Once claims are processed, evidence and recommendations will
                  appear here.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground capitalize">
                        {claim.claim_type?.replaceAll("_", " ") || "claim"}
                      </p>
                    </div>
