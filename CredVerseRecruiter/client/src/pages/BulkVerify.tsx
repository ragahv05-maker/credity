  ...
  const summary = useMemo(() => {
    return {
      ...
      suspicious: results.filter(
        (r) => r.status === "suspicious" || r.status === "pending",
      ).length,
      pass: results.filter(
        (r) => getDecisionTierFromStatus(r.status, r.riskScore) === "PASS",
      ).length,
      review: results.filter(
        (r) => getDecisionTierFromStatus(r.status, r.riskScore) === "REVIEW",
      ).length,
      fail: results.filter(
        (r) => getDecisionTierFromStatus(r.status, r.riskScore) === "FAIL",
      ).length,
    };
  }, [results]);
