...
export default function Dashboard() {
  ...
  } = useQuery<{ stats: VerificationStats }>({
  ...
  } = useQuery<{ results: VerificationRecord[] }>({
      ...
      {(statsError || recentError) && (
        <Alert className="mb-6" variant="destructive">
          ...
          <AlertDescription>
            ...
            {statsError && recentError ? " · " : ""}
            {recentError &&
              (recentErrorDetails instanceof Error
                ? recentErrorDetails.message
      ...
      <div className="grid gap-6 md:grid-cols-7 mb-8">
        <Card className="md:col-span-4 shadow-sm border-muted/40">
          <CardHeader>
            ...
            <CardDescription>
              Daily volume of processed credentials
            </CardDescription>
          </CardHeader>
          <CardContent>
          ...
          <div className="space-y-1">
            {recentVerifications.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                <p>
                  No verifications yet. Run an instant or link verification to
                  create activity.
                </p>
              </div>
            ) : (
              recentVerifications.map((record) => (
                <div
                  key={record.id}
                  className="group flex items-center justify-between border-b border-border/50 last:border-0 py-4 hover:bg-muted/30 px-2 rounded-lg transition-colors cursor-pointer"
                >
