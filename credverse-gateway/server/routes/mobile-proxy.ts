...
const TARGETS: Record<ProxyTarget, TargetConfig> = {
  ...
  issuer: {
    ...
    nonApiPrefixes: [".well-known/openid-credential-issuer"],
  },
  recruiter: {
...
const PROXY_TIMEOUT_MS = parsePositiveInt(
  process.env.MOBILE_PROXY_TIMEOUT_MS,
  10_000,
  500,
  120_000,
);
const CLAIMS_RATE_LIMIT_MAX = parsePositiveInt(
  process.env.MOBILE_PROXY_CLAIMS_RATE_LIMIT_MAX,
  60,
  1,
  1000,
);
const CLAIMS_RATE_LIMIT_WINDOW_MS = parsePositiveInt(
  process.env.MOBILE_PROXY_CLAIMS_RATE_LIMIT_WINDOW_MS,
  60_000,
  1000,
  3_600_000,
);
...
function normalizeSubpath(rawPath: string): string {
  return rawPath.replace(/^\/+/, "").replace(/\/+/g, "/").replace(/\/$/, "");
}
...
function isSubpathAllowed(target: ProxyTarget, subpath: string): boolean {
  ...
  const isApiAllowed = prefixes.some(
...
function isClaimsRoute(target: ProxyTarget, subpath: string): boolean {
  return (
    target === "wallet" &&
    (subpath === CLAIMS_BASE_PATH || subpath.startsWith(`${CLAIMS_BASE_PATH}/`))
  );
}
...
function consumeClaimsRateLimit(req: Request): {
  allowed: boolean;
...
function validateClaimsPayload(
  ...
  if (!CLAIMS_ALLOWED_METHODS.has(req.method)) {
    return {
      valid: false,
      status: 405,
      error: "Method not allowed for claims proxy route",
    };
  }
  ...
  if (queryItems.length > CLAIMS_QUERY_MAX_ITEMS) {
    return {
      valid: false,
      status: 400,
      error: "Too many query parameters for claims route",
    };
  }
    ...
    if (Array.isArray(value)) {
      if (value.length > CLAIMS_QUERY_MAX_ITEMS) {
        return {
          valid: false,
          status: 400,
          error: "Too many values for claims query parameter",
        };
      }
      if (value.some((item) => !isSafeClaimsQueryValue(item))) {
        return {
          valid: false,
          status: 400,
          error: "Claims query parameter type is not allowed",
        };
      }
      if (
        value.some((item) => String(item).length > CLAIMS_QUERY_VALUE_MAX_CHARS)
      ) {
        return {
          valid: false,
          status: 400,
          error: "Claims query parameter value too large",
        };
      }
      continue;
    }
  ...
  if (!contentType.toLowerCase().includes("application/json")) {
    return {
      valid: false,
      status: 415,
      error: "Claims route requires application/json content-type",
    };
  }
  ...
  if (payload === undefined || payload === null) {
    return {
      valid: false,
      status: 400,
      error: "Claims request payload is required",
    };
  }
  ...
  if (typeof payload !== "object" || Array.isArray(payload)) {
    return {
      valid: false,
      status: 400,
      error: "Claims request payload must be a JSON object",
    };
  }
  ...
  if (
    !serialized ||
    Buffer.byteLength(serialized, "utf8") > CLAIMS_BODY_MAX_BYTES
  ) {
    return {
      valid: false,
      status: 413,
      error: "Claims request payload too large",
    };
  }
  ...
  if (Object.keys(payload).length > 100) {
    return {
      valid: false,
      status: 400,
      error: "Claims request has too many top-level fields",
    };
  }
...
function buildTargetUrl(
  ...
  const url = new URL(
    useApiPrefix ? `${baseUrl}/api/${subpath}` : `${baseUrl}/${subpath}`,
  );
...
function mapProxyError(error: unknown): { statusCode: number; error: string } {
  ...
  if (
    ...
    ["ENOTFOUND", "ECONNREFUSED", "EHOSTUNREACH", "ECONNRESET"].includes(
      networkCode,
    )
  ) {
...
router.all("/:target/*", async (req, res) => {
  ...
  if (!isSubpathAllowed(target, subpath)) {
    return res
      .status(403)
      .json({ error: "Route not allowed by mobile proxy policy" });
  }
  ...
  if (isClaimsRoute(target, subpath)) {
    ...
    if (!claimsValidation.valid) {
      return res
        .status(claimsValidation.status)
        .json({ error: claimsValidation.error });
    }
    ...
    if (!rateLimitResult.allowed) {
      ...
      return res
        .status(429)
        .json({ error: "Too many claims requests, please retry later" });
    }
  }
