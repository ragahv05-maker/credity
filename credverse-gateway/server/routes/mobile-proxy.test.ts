...
test("mobile proxy blocks unknown targets and disallowed routes", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const unknownTarget = await localFetch(
      `${baseUrl}/api/mobile/unknown/auth/me`,
    );
    assert.equal(unknownTarget.status, 404);

    const blockedPath = await localFetch(
      `${baseUrl}/api/mobile/wallet/not-allowed/path`,
    );
    assert.equal(blockedPath.status, 403);

    const traversalPath = await localFetch(
      `${baseUrl}/api/mobile/wallet/%2e%2e/auth/login`,
    );
...
test("mobile proxy forwards issuer well-known metadata without /api prefix", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const response = await localFetch(
      `${baseUrl}/api/mobile/issuer/.well-known/openid-credential-issuer`,
    );
    assert.equal(response.status, 200);
    assert.equal(calls.length, 1);
    assert.equal(
      calls[0]?.input,
      "http://localhost:5001/.well-known/openid-credential-issuer",
    );
  });
});
...
test("mobile proxy forwards issuer OID routes with /api prefix and propagates idempotency key", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const response = await localFetch(
      `${baseUrl}/api/mobile/issuer/v1/oid4vci/credential`,
      {
        method: "POST",
        headers: {
...
test("mobile proxy forwards wallet reputation v1 routes with /api prefix", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const response = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/reputation/score?userId=1`,
    );
    assert.equal(response.status, 200);
    assert.equal(calls.length, 1);
    assert.equal(
      calls[0]?.input,
      "http://localhost:5002/api/v1/reputation/score?userId=1",
    );
  });
});
...
test("mobile proxy forwards wallet compliance routes with /api prefix", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const response = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/compliance/consents?userId=7`,
    );
    assert.equal(response.status, 200);
    assert.equal(calls.length, 1);
    assert.equal(
      calls[0]?.input,
      "http://localhost:5002/api/v1/compliance/consents?userId=7",
    );
  });
});
...
test("mobile proxy forwards issuer queue and compliance routes with /api prefix", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const queueResponse = await localFetch(
      `${baseUrl}/api/mobile/issuer/v1/queue/dead-letter?limit=5`,
    );
    assert.equal(queueResponse.status, 200);
    assert.equal(
      calls[0]?.input,
      "http://localhost:5001/api/v1/queue/dead-letter?limit=5",
    );

    const complianceResponse = await localFetch(
      `${baseUrl}/api/mobile/issuer/v1/compliance/consents`,
    );
    assert.equal(complianceResponse.status, 200);
    assert.equal(
      calls[1]?.input,
      "http://localhost:5001/api/v1/compliance/consents",
    );
  });
});
...
test("mobile proxy forwards recruiter compliance routes with /api prefix", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const response = await localFetch(
      `${baseUrl}/api/mobile/recruiter/v1/compliance/audit-log/export`,
    );
    assert.equal(response.status, 200);
    assert.equal(calls.length, 1);
    assert.equal(
      calls[0]?.input,
      "http://localhost:5003/api/v1/compliance/audit-log/export",
    );
  });
});
...
test("mobile proxy blocks malformed claims payloads before forwarding", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const arrayPayload = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/claims/verify`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
test("mobile proxy enforces claims method and content-type guardrails", async () => {
  await withProxyServer(async ({ baseUrl, localFetch, calls }) => {
    const methodRejected = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/claims/verify`,
      {
        method: "DELETE",
      },
    ...
    const contentTypeRejected = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/claims/verify`,
      {
        method: "POST",
...
test("mobile proxy rate limits claims requests per client", async () => {
  await withProxyServer(async ({ baseUrl, localFetch }) => {
    ...
    const blocked = await localFetch(
      `${baseUrl}/api/mobile/wallet/v1/claims/verify`,
      {
        method: "POST",
        headers,
...
test("mobile proxy maps upstream network and timeout errors", async () => {
  await withProxyServer(
    async ({ baseUrl, localFetch }) => {
      const unavailable = await localFetch(
        `${baseUrl}/api/mobile/wallet/v1/wallet/balance`,
      );
      assert.equal(unavailable.status, 502);
      const unavailableBody = await unavailable.json();
      assert.equal(unavailableBody.error, "Upstream unavailable");

      const timeout = await localFetch(
        `${baseUrl}/api/mobile/wallet/v1/wallet/timeout`,
      );
      assert.equal(timeout.status, 504);
      const timeoutBody = await timeout.json();
      assert.equal(timeoutBody.error, "Upstream timeout");

      const invalid = await localFetch(
        `${baseUrl}/api/mobile/wallet/v1/wallet/invalid`,
      );
