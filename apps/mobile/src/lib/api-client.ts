...
export async function requestRole<T>(
  ...
  const resolvedIdempotencyKey = shouldAttachIdempotencyKey(method)
    ? idempotencyKey || generateIdempotencyKey(role)
    : undefined;
...
export async function loginRole(
  ...
  if (data?.requires2FA) {
    throw new Error(
      "2FA-enabled account detected. Mobile 2FA flow is not implemented yet.",
    );
  }
...
export async function getHolderSafeDateScore(): Promise<any> {
  const data = await requestRole<any>(
    "holder",
    "v1/reputation/safedate?userId=1",
  );
...
export async function getHolderCredentialFields(
  id: string | number,
): Promise<{ fields: any[]; categories: any[] }> {
...
export async function createHolderDisclosure(input: {
  ...
  return requestRole(
    "holder",
    `v1/credentials/${input.credentialId}/disclose`,
    {
      method: "POST",
      body: {
...
export async function generateHolderProofMetadata(
  credentialId: string | number,
): Promise<any> {
...
const OID4VCI_PRE_AUTH_GRANT =
  "urn:ietf:params:oauth:grant-type:pre-authorized_code";

function extractPreAuthorizedCode(offerResponse: any): string | null {
  return (
    offerResponse?.credential_offer?.grants?.[OID4VCI_PRE_AUTH_GRANT]?.[
      "pre-authorized_code"
    ] || null
  );
}
...
export async function issueCredentialViaOid4vci(input: {
  ...
  try {
    const offerResponse = await requestRole<any>(
      "issuer",
    ...
    stage = "credential";
    const credentialResponse = await requestRole<any>(
      "issuer",
...
export async function getHolderConsents(userId = 1): Promise<any[]> {
  const data = await requestRole<any>(
    "holder",
    `v1/compliance/consents?userId=${encodeURIComponent(String(userId))}`,
  );
...
export async function getHolderDataRequests(userId = 1): Promise<any[]> {
  const data = await requestRole<any>(
    "holder",
    `v1/compliance/data-requests?userId=${encodeURIComponent(String(userId))}`,
  );
...
export async function revokeHolderConsent(
  consentId: string,
  userId = 1,
): Promise<any> {
  return requestRole(
    "holder",
    `v1/compliance/consents/${encodeURIComponent(consentId)}/revoke`,
    {
      method: "POST",
      body: { userId },
    },
...
export async function submitHolderDataExport(
  userId = 1,
  reason?: string,
): Promise<any> {
...
export async function submitHolderDataDelete(
  userId = 1,
  reason?: string,
): Promise<any> {
...
export async function requestIssuerDataExport(
  subjectId: string,
  reason?: string,
): Promise<any> {
...
export async function exportIssuerAuditLog(
  format: "json" | "ndjson" = "json",
): Promise<any> {
  return requestRole<any>(
    "issuer",
    `v1/compliance/audit-log/export?format=${format}`,
  );
}
...
export async function getIssuerDeadLetterEntries(limit = 25): Promise<any[]> {
  const data = await requestRole<any>(
    "issuer",
    `v1/queue/dead-letter?limit=${encodeURIComponent(String(limit))}`,
  );
  return Array.isArray(data) ? data : data?.entries || [];
}

export async function replayIssuerDeadLetterEntry(
  entryId: string,
): Promise<any> {
  return requestRole(
    "issuer",
    `v1/queue/dead-letter/${encodeURIComponent(entryId)}/replay`,
    {
      method: "POST",
    },
  );
...
export async function requestRecruiterDataExport(
  subjectId: string,
  reason?: string,
): Promise<any> {
...
export async function exportRecruiterAuditLog(
  format: "json" | "ndjson" = "json",
): Promise<any> {
  return requestRole<any>(
    "recruiter",
...
export async function getPendingConnections(): Promise<any[]> {
  try {
    ...
    const fallback = await requestRole<any>(
      "holder",
...
export async function completeLivenessSession(
  ...
  try {
    ...
    const rawScore = Number(
      data?.score ?? data?.result?.score ?? data?.result?.confidence ?? 0,
    );
    return {
    ...
    const fallback = await requestRole<any>(
      "holder",
      `v1/identity/liveness/${encodeURIComponent(sessionId)}`,
    );
    const rawScore = Number(
      fallback?.result?.score ?? fallback?.result?.confidence ?? 0,
    );
    return {
...
export async function getHolderActivity(): Promise<
  Array<{
    id: string;
    title: string;
    description: string;
...
> {
  ...
  return raw.map((item: any) => ({
    ...
    status:
      item.status === "revoked" || item.status === "pending"
        ? item.status
        : "verified",
    timestamp:
      item.timestamp ||
...
export async function revokeCredential(credentialId: string): Promise<void> {
  await requestRole(
    "holder",
    `v1/wallet/credentials/${encodeURIComponent(credentialId)}/revoke`,
    {
      method: "POST",
      body: {},
    },
