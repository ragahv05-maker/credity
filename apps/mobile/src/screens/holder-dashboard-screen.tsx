...
export function HolderDashboardScreen({ onSwitchRole, onLogout }: Props) {
  ...
  const [disclosureCredentialId, setDisclosureCredentialId] = useState<
    string | null
  >(null);
  ...
  const scoreValue =
    typeof reputation?.score === "number" ? reputation.score : null;
  const scorePercent = scoreValue
    ? Math.min(100, Math.max(0, Math.round((scoreValue / 1000) * 100)))
    : 0;
  const safeDateValue =
    typeof safeDate?.score === "number" ? safeDate.score : null;
  ...
  async function authenticateAndLoad() {
    ...
    try {
      ...
      if (!unlocked) {
        Alert.alert(
          "Unlock failed",
          "Biometric verification is required to access holder dashboard.",
        );
        return;
      }
      ...
      Alert.alert(
        "Load failed",
        error?.message || "Unable to load holder data.",
      );
    } finally {
      setUnlocking(false);
      setLoading(false);
  ...
  async function onGenerateShare() {
    if (!credentials.length) {
      Alert.alert(
        "No credential",
        "Add a credential before generating a share QR.",
      );
      return;
    }
    ...
    try {
      ...
      if (!approved) {
        Alert.alert(
          "Share blocked",
          "Biometric verification is required to share credentials.",
        );
        return;
      }
      ...
      const maybeUrl =
        share?.link ||
        share?.url ||
        share?.verificationUrl ||
        JSON.stringify(share);
      setLastShareLink(String(maybeUrl));
    } catch (error: any) {
      Alert.alert(
        "Share failed",
        error?.message || "Unable to generate QR/share link.",
      );
    }
  }
  ...
  async function onSelectCredential(id: string | number) {
    try {
      ...
      Alert.alert(
        "Detail failed",
        error?.message || "Unable to load credential detail.",
      );
    }
  }
  ...
  async function onClaimOffer() {
    ...
    if (!url) {
      Alert.alert(
        "Offer required",
        "Paste an offer URL from the Issuer to claim a credential.",
      );
      return;
    }
    ...
    try {
      ...
      if (!approved) {
        Alert.alert(
          "Claim blocked",
          "Biometric verification is required to claim credentials.",
        );
        return;
      }
      ...
      Alert.alert(
        "Credential claimed",
        result?.message || "Credential stored in wallet.",
      );
    } catch (error: any) {
      Alert.alert(
        "Claim failed",
        error?.message || "Unable to claim credential offer.",
      );
    } finally {
      setClaimBusy(false);
    }
  ...
  async function onOpenDisclosure(credentialId: string | number) {
    try {
      ...
      Alert.alert(
        "Fields failed",
        error?.message || "Unable to load credential fields.",
      );
    }
  }
  ...
  async function onGenerateRecruiterPackage() {
    ...
    try {
      const approved = await requireProtectedAction(
        "Generate selective disclosure package",
      );
      if (!approved) {
        Alert.alert(
          "Share blocked",
      ...
      const payload = {
        ...
        credential:
          disclosure?.token?.disclosedData ||
          disclosure?.token?.disclosed_data ||
          null,
      ...
      Alert.alert(
        "Package failed",
        error?.message || "Unable to generate recruiter package.",
      );
    }
  }
  ...
  async function onRequestExport() {
    try {
      ...
      if (!approved) {
        Alert.alert(
          "Request blocked",
          "Biometric verification is required for export requests.",
        );
        return;
      }
      ...
      Alert.alert(
        "Request failed",
        error?.message || "Unable to submit data export request.",
      );
    }
  }
  ...
  async function onRequestDelete() {
    try {
      ...
      if (!approved) {
        Alert.alert(
          "Request blocked",
          "Biometric verification is required for deletion requests.",
        );
        return;
      }
      ...
      Alert.alert(
        "Request failed",
        error?.message || "Unable to submit data deletion request.",
      );
    }
  }
  ...
  async function onRevokeFirstConsent() {
    ...
    try {
      const approved = await requireProtectedAction(
        "Approve consent revocation",
      );
      if (!approved) {
        Alert.alert(
          "Revocation blocked",
          "Biometric verification is required to revoke consent.",
        );
        return;
      }
      ...
      Alert.alert(
        "Revocation failed",
        error?.message || "Unable to revoke consent.",
      );
    }
  }
  ...
  return (
    <>
      ...
      >
        <View style={styles.headerRow}>
          <View>
            ...
            <Text style={styles.subtitle}>
              Manage credentials, share proofs, and control access.
            </Text>
          </View>
        ...
        <View style={styles.scoreCard}>
          ...
          <View style={styles.progressTrack}>
            <View
              style={[styles.progressFill, { width: `${scorePercent}%` }]}
            />
          </View>
          <Text style={styles.scoreHint}>
            Improve your score by linking verified platforms.
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.metric}>
            Credentials: {summary.credentialCount}
          </Text>
          <Text style={styles.metric}>
            Trust Score: {String(summary.trust)}
          </Text>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.metricRow}>
            ...
            <Text style={styles.metricValue}>
              {String(reputation?.event_count ?? 0)}
            </Text>
          </View>
          <View style={styles.metricRow}>
          ...
          <Text style={styles.meta}>
            Reason Codes:{" "}
            {Array.isArray(safeDate?.reason_codes) &&
            safeDate.reason_codes.length
              ? safeDate.reason_codes.join(", ")
              : "none"}
          </Text>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.meta}>
            Paste an Issuer offer URL (or deep link URL parameter) to import a
            VC into your wallet.
          </Text>
          <TextInput
            style={styles.input}
          ...
          >
            <Text style={styles.primaryButtonText}>
              {claimBusy ? "Claiming…" : "Claim & store"}
            </Text>
          </Pressable>
        </View>
        ...
        <View style={styles.card}>
          ...
          {!credentials.length ? (
            <Text style={styles.meta}>No credentials available.</Text>
          ) : null}
          {credentials.slice(0, 5).map((cred) => (
            <Pressable
              key={String(cred.id)}
              style={styles.rowItem}
              onPress={() => onSelectCredential(cred.id)}
            >
              <Text style={styles.rowTitle}>#{cred.id}</Text>
              <Text style={styles.meta}>
                {cred.type || cred.templateId || "Credential"}
              </Text>
            </Pressable>
          ))}
          ...
          {lastShareLink ? (
            <Text style={styles.meta}>Last Share: {lastShareLink}</Text>
          ) : null}
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.metric}>
            CERT-In Incidents: {certInIncidents.length}
          </Text>
          <Text style={styles.meta}>
            Latest Request: {dataRequests[0]?.request_type || "none"} /{" "}
            {dataRequests[0]?.status || "n/a"}
          </Text>
        ...
        {selectedCredential ? (
          <View style={styles.card}>
            ...
            <Text style={styles.meta}>
              Type:{" "}
              {selectedCredential?.credential?.type ||
                selectedCredential?.credential?.templateId ||
                selectedCredential?.credential?.data?.vc?.type?.[1] ||
                "n/a"}
            </Text>
            <Text style={styles.meta}>
              Issuer:{" "}
              {selectedCredential?.credential?.issuer ||
                selectedCredential?.credential?.issuerDid ||
                "n/a"}
            </Text>
            <Text style={styles.meta}>
              Status:{" "}
              {selectedCredential?.credential?.status ||
                (selectedCredential?.credential?.revoked
                  ? "revoked"
                  : "active")}
            </Text>
            <Text style={styles.meta}>
              Anchor:{" "}
              {selectedCredential?.credential?.anchorStatus ||
                selectedCredential?.credential?.data?.proof?.code ||
                "unknown"}
            </Text>
            <Pressable
              style={styles.primaryButton}
              onPress={() =>
                onOpenDisclosure(
                  selectedCredential?.credential?.id || selectedCredential?.id,
                )
              }
            >
              <Text style={styles.primaryButtonText}>
                Selective disclose (recruiter package)
              </Text>
            </Pressable>
          </View>
        ) : null}
        ...
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalSubtitle}>
              Choose which fields to disclose. We generate a deterministic proof
              hash for auditability.
            </Text>

            {!availableFields.length ? (
              <Text style={styles.modalHint}>
                No fields detected for this credential.
              </Text>
            ) : (
              <View style={{ gap: 8 }}>
                  ...
                  return (
                    <Pressable
                      ...
                      style={[
                        ...
                        selected ? styles.disclosureRowSelected : null,
                      ]}
                      onPress={() => {
                    ...
                    >
                      <Text style={styles.disclosureRowText}>
                        {selected ? "✓ " : ""}
                        {field}
                      </Text>
                    </Pressable>
            ...
            <View style={styles.modalActions}>
              ...
              <Pressable
                style={styles.primaryButton}
                onPress={onGenerateRecruiterPackage}
              >
            ...
            {sharePayload ? (
              <View style={{ gap: 8 }}>
                ...
                />
                <Text style={styles.modalHint}>
                  Recruiter: paste this JSON into Recruiter → Instant
                  Verification (it auto-detects JSON vs JWT).
                </Text>
              </View>
            ) : null}
          </View>
      ...
      <Modal
        ...
        visible={showZkProof}
        animationType="slide"
        onRequestClose={onCloseZkProof}
      >
        <View style={styles.modalBackdrop}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>Selective disclosure</Text>
            <Text style={styles.modalSubtitle}>
              Prove a claim without revealing private data. The verifier only
              learns the claim is true.
            </Text>

            <View style={styles.modalSection}>
              <Text style={styles.modalLabel}>Claim</Text>
              <Text style={styles.modalValue}>
                WorkScore is above required threshold
              </Text>
              <Text style={styles.modalMeta}>
                Your exact score stays private.
              </Text>
            </View>
            ...
            <View style={styles.modalSection}>
              ...
              <Text style={styles.modalMeta}>
                • No raw score or identity data
              </Text>
            </View>

            <Text style={styles.modalHint}>
              You can revoke access anytime from your activity log or privacy
              settings.
            </Text>
