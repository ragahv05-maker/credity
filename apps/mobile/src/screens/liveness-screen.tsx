...
function normalizeCompletionResult(payload: any): LivenessResultState {
  ...
  const success = Boolean(
    payload?.passed ?? payload?.verified ?? payload?.success,
  );
  return {
    success,
...
export function LivenessScreen() {
  ...
  const [phase, setPhase] = useState<
    "booting" | "capturing" | "submitting" | "result"
  >("booting");
  const [sessionId, setSessionId] = useState<string | null>(null);
  const [challenges, setChallenges] =
    useState<LivenessChallenge[]>(FALLBACK_CHALLENGES);
  ...
  const activeChallenge =
    ...
    FALLBACK_CHALLENGES[activeIndex] ||
      ...
      setResult({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to start liveness session.",
      });
      setPhase("result");
    }
    ...
    try {
      ...
      setResult({
        ...
        message:
          error instanceof Error
            ? error.message
            : "Unable to submit liveness challenge.",
      });
      setPhase("result");
    }
  }
    ...
    return (
      <View style={styles.permissionContainer}>
        ...
        <Text style={styles.permissionText}>
          Enable camera access to complete liveness verification.
        </Text>
        <Pressable
      ...
      <View style={styles.challengeCard}>
        ...
        <View style={styles.dotRow}>
          {[0, 1, 2].map((dot) => (
            <View
              key={dot}
              style={[styles.dot, dot === activeIndex && styles.dotActive]}
            />
          ))}
        </View>
        ...
        <View style={styles.timerWrap}>
          <Animated.View
            style={[styles.timerFill, { width: countdownBarWidth }]}
          />
        </View>
        ...
        <Pressable
          style={[
            ...
            phase === "submitting" && styles.disabledButton,
          ]}
          onPress={() => void onCompleteChallenge()}
          disabled={phase === "submitting"}
        >
          <Text style={styles.primaryButtonText}>
            {phase === "submitting" ? "Submitting..." : "Done"}
          </Text>
        </Pressable>
      </View>
    </View>
