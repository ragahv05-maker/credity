...
const BACKEND_URL = (
  process.env.EXPO_PUBLIC_WALLET_URL ?? "http://localhost:5000"
).replace(/\/$/, "");
...
export function AuthScreen() {
  ...
  async function onGoogleSignIn() {
    try {
      ...
      Alert.alert(
        ...
        err?.message || "Unable to open Google sign-in.",
      );
    }
  }
  ...
  async function onAppleSignIn() {
    ...
    if (role !== "holder") {
      Alert.alert(
        "Holder only",
        "Apple Sign-In is currently supported for Holder Wallet accounts only.",
      );
      return;
    }
