...
export function IssuerDashboardScreen({ onSwitchRole, onLogout }: Props) {
  ...
  const [lastIssueMode, setLastIssueMode] = useState<
    "oid4vci" | "legacy" | null
  >(null);
  const [lastIssueCredentialId, setLastIssueCredentialId] = useState<
    string | null
  >(null);
  ...
  async function refresh() {
    ...
    try {
      ...
      Alert.alert(
        "Load failed",
        error?.message || "Unable to load issuer data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function onIssue() {
    const payload = {
      ...
      recipient: {
        ...
        name: recipientEmail.split("@")[0] || "Recipient",
      },
      credentialData: {
        source: "mobile-mvp",
    ...
    try {
      ...
      if (!approved) {
        Alert.alert(
          "Issuance blocked",
          "Biometric verification is required to issue credentials.",
        );
        return;
      }
      ...
      try {
        ...
        const fallbackAllowed =
          fallbackReason.includes("[oid4vci:offer]") ||
          fallbackReason.includes("[oid4vci:token]");
      ...
      Alert.alert(
        "Issue failed",
        error?.message || "Unable to issue credential.",
      );
    }
  }
  ...
  async function onReplayFirstDeadLetter() {
    ...
    try {
      const approved = await requireProtectedAction(
        "Replay queue dead-letter job",
      );
      if (!approved) {
        Alert.alert(
          "Replay blocked",
          "Biometric verification is required to replay queue jobs.",
        );
        return;
      }
      ...
      setLastReplayJob(
        String(replay?.jobId || replay?.replayedJobId || "queued"),
      );
      await refresh();
      ...
      Alert.alert(
        "Replay failed",
        error?.message || "Unable to replay dead-letter entry.",
      );
    }
  }
  ...
  async function onComplianceExport() {
    try {
      const response = await requestIssuerDataExport(
        recipientEmail.trim() || "mobile-subject",
        "mobile_issuer_export",
      );
      Alert.alert(
        "Export requested",
        `Request ID: ${response?.id || "accepted"}`,
      );
    } catch (error: any) {
      Alert.alert(
        "Export failed",
        error?.message || "Unable to submit export request.",
      );
    }
  }
  ...
  async function onAuditExport() {
    try {
      ...
      setAuditIntegrity(
        typeof valid === "boolean" ? (valid ? "valid" : "invalid") : "unknown",
      );
      Alert.alert(
        "Audit export ready",
        `Events exported: ${exported?.count ?? 0}`,
      );
    } catch (error: any) {
      Alert.alert(
        "Audit export failed",
        error?.message || "Unable to export audit log.",
      );
    }
  }
  ...
  async function onRevokeFirstConsent() {
    ...
    try {
      const approved = await requireProtectedAction(
        "Approve issuer consent revocation",
      );
      if (!approved) {
        Alert.alert(
          "Revocation blocked",
      ...
      Alert.alert(
        "Consent revoked",
        `Consent ${consentId} revoked successfully.`,
      );
    } catch (error: any) {
      Alert.alert(
        "Revocation failed",
    ...
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={styles.subtitle}>
            Issue, revoke, and audit verifiable credentials.
          </Text>
        </View>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.meta}>
          Preferred flow: OID4VCI (auto-fallback to legacy)
        </Text>
        {lastIssueMode ? (
          <Text style={styles.meta}>Last issue mode: {lastIssueMode}</Text>
        ) : null}
        {lastIssueCredentialId ? (
          <Text style={styles.meta}>
            Last credential ID: {lastIssueCredentialId}
          </Text>
        ) : null}
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.meta}>
          Queue available:{" "}
          {queueStats?.queue?.available === false ? "no" : "yes"}
        </Text>
        <Text style={styles.meta}>
          Dead-letter entries: {deadLetterEntries.length}
        </Text>
        <Text style={styles.meta}>
          Last replay job: {lastReplayJob || "none"}
        </Text>
        <Pressable
          style={styles.primaryButton}
          onPress={onReplayFirstDeadLetter}
        >
          <Text style={styles.primaryButtonText}>
            Replay latest dead-letter
          </Text>
        </Pressable>
      </View>
      ...
      <View style={styles.card}>
        ...
        <Text style={styles.meta}>
          Active consents: {complianceConsents.length}
        </Text>
        ...
        <Text style={styles.meta}>
          Audit chain: {auditIntegrity || "not exported"}
        </Text>
