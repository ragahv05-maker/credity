...
export function RecruiterDashboardScreen({ onSwitchRole, onLogout }: Props) {
  ...
  async function refresh() {
    ...
    try {
      ...
      Alert.alert(
        "Load failed",
        error?.message || "Unable to load recruiter data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function onVerify() {
    if (!jwtInput.trim()) {
      Alert.alert(
        "Input required",
        "Paste JWT or credential payload to verify.",
      );
      return;
    }
    ...
    try {
      ...
      if (!approved) {
        Alert.alert(
          ...
          "Biometric verification is required for instant verify.",
        );
        return;
      }
      ...
      if (trimmed.startsWith("{") || trimmed.startsWith("[")) {
        ...
        try {
          ...
          throw new Error(
            "Invalid JSON payload. Paste a JWT string or a JSON object.",
          );
        }
      ...
      Alert.alert(
        "Verify failed",
        error?.message || "Unable to run instant verification.",
      );
    }
  }
  ...
  async function onSelectRecord(id: string) {
    try {
      ...
      Alert.alert(
        "Detail failed",
        error?.message || "Unable to fetch verification detail.",
      );
    }
  }
  ...
  async function onComplianceExport() {
    const subjectId =
      selectedVerification?.subject || profile?.username || "mobile-subject";
    try {
      await requestRecruiterDataExport(
        String(subjectId),
        "mobile_recruiter_export",
      );
      Alert.alert("Export requested", `Submitted for subject: ${subjectId}`);
    } catch (error: any) {
      Alert.alert(
        "Export failed",
        error?.message || "Unable to submit recruiter export request.",
      );
    }
  }
  ...
  async function onAuditExport() {
    try {
      ...
      Alert.alert(
        "Audit export failed",
        error?.message || "Unable to export recruiter audit log.",
      );
    }
  }
  ...
  async function onRevokeFirstConsent() {
    ...
    try {
      const approved = await requireProtectedAction(
        "Approve recruiter consent revocation",
      );
      if (!approved) {
        Alert.alert(
          "Revocation blocked",
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content}>
      <View style={styles.headerRow}>
        <View>
          ...
          <Text style={styles.subtitle}>
            Run checks with clear, auditable risk signals.
          </Text>
        </View>
      ...
      <View style={styles.card}>
        ...
        {lastDecision ? (
          <Text style={styles.meta}>Last decision: {lastDecision}</Text>
        ) : null}
      </View>
      ...
      <View style={styles.card}>
        ...
        {!records.length ? (
          <Text style={styles.meta}>No verification history available.</Text>
        ) : null}
        {records.slice(0, 5).map((row) => (
          <Pressable
            key={String(row.id)}
            style={styles.rowItem}
            onPress={() => onSelectRecord(String(row.id))}
          >
            <Text style={styles.rowTitle}>
              {row.subject || "Unknown subject"}
            </Text>
            <Text style={styles.meta}>
              {row.status || row.recommendation || "pending"}
            </Text>
          </Pressable>
        ))}
      ...
      {selectedVerification ? (
        <View style={styles.card}>
          ...
          <Text style={styles.meta}>
            ID: {selectedVerification?.id || "n/a"}
          </Text>
          <Text style={styles.meta}>
            Status: {selectedVerification?.status || "n/a"}
          </Text>
          <Text style={styles.meta}>
            Issuer: {selectedVerification?.issuer || "n/a"}
          </Text>
          <Text style={styles.meta}>
            Subject: {selectedVerification?.subject || "n/a"}
          </Text>
          <Text style={styles.meta}>
            Risk Score:{" "}
            {String(
              selectedVerification?.risk_score ??
          ...
          <Text style={styles.meta}>
            Fraud Score:{" "}
            {String(
              selectedVerification?.fraud_score ??
                selectedVerification?.fraudScore ??
                "n/a",
            )}
          </Text>
          <Text style={styles.meta}>
            Recommendation: {selectedVerification?.recommendation || "n/a"}
          </Text>
        </View>
      ) : null}
    </ScrollView>
