...
## 2024-10-25 - BlockWalletDigi Frontend Verification Server Port
**Learning:** Found that running `npm run dev` in `BlockWalletDigi` primarily starts the backend. The correct script to start the frontend Vite server on port 5000 is `npm run dev:client`.
**Action:** Always check `package.json` for specific client/frontend run scripts (`dev:client`) and verify the active port when setting up Playwright for visual verification.
