...
## 2024-10-25 - CredVerseRecruiter Tooltip Accessibility
**Learning:** Found that `shadcn/ui` Tooltip implementation requires both `TooltipTrigger` and `TooltipContent` to function correctly and remain accessible. In header layouts, multiple individual tooltips can be wrapped in their own `TooltipProvider` without causing layout issues.
**Action:** When adding tooltips to icon-only buttons, always wrap them in `TooltipTrigger` and provide descriptive `TooltipContent` for screen readers and hover states.
