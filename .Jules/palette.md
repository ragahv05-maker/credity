...
## 2024-10-25 - React Tooltip Accessibility in Header
**Learning:** In shadcn/ui components (Radix), wrapping existing `<Button>` components with `TooltipTrigger` directly creates invalid nested buttons.
**Action:** When adding tooltips to existing buttons, always use `<TooltipTrigger asChild>` to properly forward refs and avoid nesting issues, and ensure descriptive `aria-label`s are kept on the interactive element itself.
