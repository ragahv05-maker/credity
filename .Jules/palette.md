...
## 2024-10-25 - npm/pnpm Interoperability in Monorepo CI
**Learning:** Using `npm ci` directly inside monorepo packages (e.g., `CredVerseRecruiter`, `CredVerseIssuer 3`) without workspaces enabled can wipe out local root-level hoisted dependencies (like `shared-auth`) and create duplicate `package-lock.json` files, causing CI build scripts and tests relying on cross-package imports to fail with `Error [ERR_PACKAGE_PATH_NOT_EXPORTED]` or `Cannot find package '@credverse/shared-auth'`.
**Action:** When fixing dependency vulnerabilities or resolving missing modules in a specific package, explicitly use `npm install --legacy-peer-deps` within that subpackage directory instead of `npm ci`, and always explicitly verify that tests still pass, paying special attention to cross-package dependencies. Never leave newly generated `package-lock.json` files in subpackages uncleaned before a commit.
