...
## 2024-10-25 - Tooltip Pattern Exploration
**Learning:** Adding fully accessible tooltips using `shadcn/ui` requires `TooltipProvider`, `Tooltip`, `TooltipTrigger`, and `TooltipContent`, which is too extensive for a 50-line minor UI polish constraint.
**Action:** When constrained by line limits for accessibility improvements, prioritize adding `aria-label` attributes directly to icon-only buttons over implementing full visual tooltips, as it still achieves the core accessibility goal for screen readers.
