...
describe('digilocker production policy', () => {
  ...
  afterEach(() => {
    ...
    if (originalClientSecret === undefined) delete process.env.DIGILOCKER_CLIENT_SECRET;
  ...
  it('throws at startup in production when DigiLocker credentials are missing', () => {
    ...
    expect(() => new DigiLockerService()).toThrow('Missing DIGILOCKER_CLIENT_ID / DIGILOCKER_CLIENT_SECRET in production');
  });
});
