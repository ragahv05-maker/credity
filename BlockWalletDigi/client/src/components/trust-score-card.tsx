...
interface TrustScoreData {
  ...
  breakdown: {
    ...
    reputation: {
      total: number;
      maxPoints: number;
      noSuspiciousActivity: number;
      platformEndorsements: number;
      userFeedback: number;
    };
  };
  suggestions: Array<{
...
export function TrustScoreCard() {
  ...
  return (
    ...
    >
      {/* Main Score Display */}
      <div
        className={`bg-gradient-to-br ${getLevelColor(data.level)} p-6 text-white relative overflow-hidden`}
      >
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-sm">
        <div className="flex items-center gap-2">
          <span className={`${colorClasses[color].replace("bg-", "text-")}`}>
            {icon}
          </span>
