...
function isLiveModeEnabled(): boolean {
  return (
    String(import.meta.env.VITE_REPUTATION_PREVIEW_LIVE || "").toLowerCase() ===
    "true"
  );
}
...
export default function ReputationContractPreview() {
  ...
  const [data, setData] = useState<ReputationPreviewPayload>(
    mockReputationPreviewPayload,
  );
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">
              Consumer wallet placeholder view for WorkScore + SafeDate shared
              contract rendering.
            </p>
          </div>
          ...
          <div className="rounded-xl border bg-card p-5 space-y-3">
            ...
            <p className="text-sm text-muted-foreground">
              Candidate: {candidate.candidate_id} • Confidence:{" "}
              {(candidate.confidence * 100).toFixed(0)}% • Risk:{" "}
