...
export default function IdentityVerification() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "liveness" | "biometrics" | "documents"
  >("overview");
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description:
            "Your device biometrics are now linked to your identity.",
        });
        queryClient.invalidateQueries({ queryKey: ["identity-status"] });
      } else {
        toast({
          ...
          description:
            result.error || "Could not complete biometric enrollment",
          variant: "destructive",
        });
      }
    } catch (error: any) {
  ...
  const completeLiveness = async (frameData?: string | null) => {
    try {
      ...
      if (data.success) {
        toast({
          ...
          description: data.aiAnalysis
            ? "AI confirmed identity and liveness."
            : "Your face has been verified as a real person.",
        });
      ...
      toast({
        title: "Verification Failed",
        description:
          error.message || "Could not complete liveness verification",
        variant: "destructive",
      });
    }
  };
  ...
  const getChallengeIcon = (type: string) => {
    switch (type) {
      ...
      case "turn_left":
        return <MoveLeft className="w-12 h-12" />;
      case "turn_right":
        return <MoveRight className="w-12 h-12" />;
      case "nod":
        return <ArrowDown className="w-12 h-12" />;
      default:
          ...
          <div className="flex bg-secondary/30 rounded-xl p-1 relative">
            {[
              ...
              <button
                ...
                className={`flex-1 relative flex items-center justify-center gap-2 py-3 px-4 rounded-lg text-sm font-medium transition-all duration-300 z-10 ${
                  activeTab === tab.id
                    ? "text-primary"
              ...
              >
                ...
                <tab.icon
                  className={`w-4 h-4 ${activeTab === tab.id ? "text-primary" : ""}`}
                />
                {tab.label}
              </button>
            ))}
            ...
            {activeTab === "overview" && (
              ...
              >
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  {/* Liveness */}
                  <div
                    className={`p-6 rounded-xl border ${status?.liveness?.verified ? "bg-green-50 border-green-200" : "bg-card"}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <Camera
                        className={`w-8 h-8 ${status?.liveness?.verified ? "text-green-600" : "text-muted-foreground"}`}
                      />
                    ...
                    <Button
                      className="w-full"
                      variant={
                        status?.liveness?.verified ? "outline" : "default"
                      }
                  ...
                  {/* Biometrics */}
                  <div
                    className={`p-6 rounded-xl border ${status?.biometrics?.enrolled ? "bg-green-50 border-green-200" : "bg-card"}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <Fingerprint
                        className={`w-8 h-8 ${status?.biometrics?.enrolled ? "text-green-600" : "text-muted-foreground"}`}
                      />
                    ...
                    <Button
                      className="w-full"
                      variant={
                        status?.biometrics?.enrolled ? "outline" : "default"
                      }
                  ...
                  {/* Documents */}
                  <div
                    className={`p-6 rounded-xl border ${status?.documents?.verified ? "bg-green-50 border-green-200" : "bg-card"}`}
                  >
                    <div className="flex items-center justify-between mb-4">
                      <FileText
                        className={`w-8 h-8 ${status?.documents?.verified ? "text-green-600" : "text-muted-foreground"}`}
                      />
                      {status?.documents?.verified ? (
                        <Badge className="bg-green-100 text-green-700">
                          {status.documents.count} Verified
                        </Badge>
                      ) : (
                    ...
                    <Button
                      className="w-full"
                      variant={
                        status?.documents?.verified ? "outline" : "default"
                      }
                  ...
                  {!livenessSession ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-sm text-muted-foreground mb-6">
                        You'll be asked to perform 3 simple actions to verify
                    ...
                    <div className="relative space-y-4">
                      ...
                      <div className="grid grid-cols-3 gap-3 px-4">
                        {livenessSession.challenges?.map(
                          (c: Challenge, i: number) => {
                            const isCompleted =
                              i <
                              (livenessSession.challenges?.findIndex(
                                (ch: Challenge) =>
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="group relative p-6 rounded-xl border bg-card/50 hover:border-primary/50 transition-all duration-300 hover:shadow-lg cursor-pointer overflow-hidden">
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-sm text-muted-foreground mb-4 leading-relaxed">
                          Securely authenticate using hardware-backed facial
                          recognition.
                        </p>
                        <Button
                          className="w-full"
                          variant={
                            status?.biometrics?.type === "face_id"
                              ? "secondary"
                              : "default"
                          }
                    ...
                    <div className="group relative p-6 rounded-xl border bg-card/50 hover:border-primary/50 transition-all duration-300 hover:shadow-lg cursor-pointer overflow-hidden">
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-sm text-muted-foreground mb-4 leading-relaxed">
                          Use your fingerprint sensor for instant, high-security
                          access.
                        </p>
                        <Button
                          className="w-full"
                          variant={
                            status?.biometrics?.type === "fingerprint"
                              ? "secondary"
                              : "default"
                          }
                  ...
                  <div className="space-y-6">
                    <div className="flex items-start gap-6 p-6 bg-gradient-to-br from-blue-600/5 to-blue-600/10 border border-blue-200/50 dark:border-blue-500/20 rounded-2xl relative overflow-hidden">
                      ...
                      <div className="flex-1 relative z-10">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <Badge
                            variant="outline"
                            className="bg-blue-100/50 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800"
                          >
                            Government Issued
                          </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                          Securely fetch and verify your government-issued
                          documents directly from the source. Uses W3C
                          Verifiable Credentials standard.
                        </p>

                        <div className="flex flex-wrap gap-2 mb-6">
                          {["Aadhaar", "PAN Card", "Driving License"].map(
                            (doc) => (
                              <span
                                key={doc}
                                className="text-xs bg-background/50 border px-2.5 py-1 rounded-md text-muted-foreground font-mono"
                              >
                                {doc}
                              </span>
                            ),
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {[
                        ...
                        <div
                          key={i}
                          className="flex flex-col items-center justify-center p-4 bg-secondary/20 rounded-xl border border-border/50 text-center gap-2"
                        >
                    ...
                    <p className="text-xs text-center text-muted-foreground/60 pt-4 border-t border-border/50">
                      ...
                      Your data is encrypted end-to-end. We never store your raw
                      documents.
                    </p>
                  </div>

                  {/* Verified Documents */}
                  {status?.documents?.types &&
                    status.documents.types.length > 0 && (
