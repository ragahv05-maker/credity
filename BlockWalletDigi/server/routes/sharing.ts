...
import {
  createProofMetadata,
  verifyDeterministicProof,
} from "../utils/proof-lifecycle";
...
function requireProofAuth(allowedRoles: AuthUser["role"][]) {
  return (req: any, res: any, next: any) => {
    ...
    if (!allowedRoles.includes(payload.role as AuthUser["role"])) {
      return res
        .status(403)
        .json({ error: "Insufficient permissions", code: "PROOF_FORBIDDEN" });
    }
...
router.post(
  ...
  requireProofAuth(["holder", "admin"]),
  async (req, res) => {
    try {
      const userId =
        Number.parseInt(
          String(req.user?.userId || req.body?.userId || "1"),
          10,
        ) || 1;
...
router.post(
  ...
  requireProofAuth(["holder", "verifier", "issuer", "admin"]),
  async (req, res) => {
    try {
      ...
      return res
        .status(500)
        .json({ error: "Failed to verify proof", code: "PROOF_VERIFY_FAILED" });
    }
  },
);
...
router.post("/wallet/share", async (req, res) => {
  try {
    ...
    if (!userId || !credentialId) {
      return res
        .status(400)
        .json({ error: "userId and credentialId required" });
    }
...
router.get("/wallet/credentials/:id/fields", async (req, res) => {
  try {
    ...
    const fields = selectiveDisclosureService.getCredentialFields(
      credential.data,
    );
...
router.post("/credentials/:id/disclose", async (req, res) => {
  try {
    ...
    if (!requestedFields || !Array.isArray(requestedFields)) {
      return res
        .status(400)
        .json({ error: "requestedFields array is required" });
    }
