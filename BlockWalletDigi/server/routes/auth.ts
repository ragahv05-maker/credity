...
router.post("/auth/login", async (req, res) => {
  try {
    ...
    if (!passwordHash) {
      ...
      console.warn(
        "[Auth] Allowing legacy no-password login because ALLOW_DEMO_ROUTES=true",
      );
    } else {
...
router.post("/auth/change-password", authMiddleware, async (req, res) => {
  try {
    ...
    await storage.updateUser(Number(req.user!.userId), {
      password: newHash,
    } as any);
...
router.post("/auth/apple", async (req, res) => {
  try {
    ...
    const email =
      payload.email ?? `apple_${payload.sub}@privaterelay.appleid.com`;
...
router.get("/auth/apple/callback", (req, res) => {
  ...
  if (!code && !idToken) {
    return res
      .status(400)
      .json({ error: "Apple callback missing code or id_token" });
  }
    ...
    new GoogleStrategy(
      ...
      (
        ...
        profile: import("passport-google-oauth20").Profile,
          ...
          err: null,
          user: import("passport-google-oauth20").Profile,
        ) => void,
      ) => done(null, profile),
    ),
  );
...
router.get(
  ...
  async (req, res) => {
    try {
      const profile =
        req.user as unknown as import("passport-google-oauth20").Profile;
      ...
      res.redirect(
        `${FRONTEND_URL}/?token=${accessToken}&refresh=${refreshToken}`,
      );
    } catch (err) {
...
router.post("/auth/reset-password", async (req, res) => {
  ...
  if (!parsed.success)
    return res
      .status(400)
      .json({
        error: "email, 6-digit code, and newPassword (min 8 chars) required",
      });
  try {
...
router.post("/auth/pin/setup", authMiddleware, async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({ error: "pin must be numeric and 4-8 digits" });
    }
...
router.post("/auth/pin/verify", async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      return res
        .status(400)
        .json({ error: "username and valid pin are required" });
    }
    ...
    if (!user || !hasPin(user.id)) {
      return res
        .status(401)
        .json({ error: "PIN fallback unavailable for this user" });
    }
