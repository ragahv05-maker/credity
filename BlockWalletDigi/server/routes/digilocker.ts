...
router.post("/digilocker/import-all", authMiddleware, async (req, res) => {
    try {
        ...
        if (credentialsToStore.length > 0) {
            // ⚡ Bolt: Optimize batch insertion by using storeCredentials instead of looping storeCredential
            // This prevents triggering queuePersist sequentially for every imported credential
