...
router.post("/digilocker/import-all", authMiddleware, async (req, res) => {
  try {
    ...
    if (credentialsToStore.length > 0) {
      // OPTIMIZATION: Batched credential storage
      // Replaced sequential N+1 storeCredential calls with a single storeCredentials call.
      // Expected impact: Reduces database I/O from O(N) to O(1) persistence query.
