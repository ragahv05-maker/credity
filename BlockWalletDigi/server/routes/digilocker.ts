...
router.post("/digilocker/import-all", authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt Optimization: Parallelize fetching and storing from digilocker to speed up document import
    const results = await Promise.allSettled(
      documents.map(async (doc) => {
        try {
          ...
          return { success: false as const, doc };
        }
      }),
    );
    ...
    for (const result of results) {
      if (result.status === "fulfilled") {
        if (result.value.success) {
          successDocs.push(result.value.doc.name);
        } else {
          failed.push(result.value.doc.name);
        }
      } else {
        failed.push("Unknown Document");
      }
    }
