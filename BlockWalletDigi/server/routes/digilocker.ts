...
router.post("/digilocker/import-all", authMiddleware, async (req, res) => {
    try {
        ...
        if (credentialsToStore.length > 0) {
            ...
            // Use batched storeCredentials instead of sequential storeCredential in a loop
            // to avoid N+1 database queries/writes and significantly improve import speed.
