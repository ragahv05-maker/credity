...
router.get("/connections/digilocker/auth", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        error: error.message || "Failed to initiate DigiLocker user-pull auth",
      });
  }
});
...
router.get("/connections/digilocker/callback", async (req, res) => {
  const frontendBaseUrl = (
    process.env.BLOCKWALLET_URL || "http://localhost:5000"
  ).replace(/\/+$/, "");
  ...
  try {
    ...
    if (error) {
      return res.redirect(
        `${frontendPath}?digilocker=error&reason=${encodeURIComponent(String(error))}`,
      );
    }

    if (!code || !state) {
      return res.redirect(
        `${frontendPath}?digilocker=error&reason=missing_params`,
      );
    }
    ...
    res.redirect(
      `${frontendPath}?digilocker=error&reason=${encodeURIComponent(error.message || "callback_failed")}`,
    );
  }
});
 ...
 */
router.get(
  "/connections/digilocker/documents",
  authMiddleware,
  async (req, res) => {
    try {
      const userId = getAuthenticatedUserId(req, res);
      ...
      res
        .status(500)
        .json({
          error: error.message || "Failed to list DigiLocker documents",
        });
    }
  },
);
  ...
  async (req, res) => {
    try {
      ...
      if (!documentUri || !documentType) {
        return res
          .status(400)
          .json({ error: "documentUri and documentType are required" });
      }
      ...
      const pulled = await pullUserPullDocument(
        tokens.accessToken,
        String(documentUri),
      );
      ...
      res
        .status(500)
        .json({
          error: error.message || "Failed to import DigiLocker document",
        });
    }
  },
);
  ...
  try {
    ...
    if (String(error?.message || "").includes("not configured")) {
      return res
        .status(503)
        .json({
          error: "DigiLocker is not configured for this environment",
          code: "DIGILOCKER_UNAVAILABLE",
        });
    }
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_AUTH_URL_FAILED" });
  }
});
...
router.get("/digilocker/callback", async (req, res) => {
  try {
    ...
    if (error) {
      ...
      return res.redirect(
        `/connect-digilocker?error=${encodeURIComponent(error as string)}`,
      );
    }
    ...
    res.redirect(
      `/connect-digilocker?connected=true&digilocker_id=${tokens.digilocker_id || ""}`,
    );
  } catch (error: any) {
    console.error("[DigiLocker] Callback error:", error);
    res.redirect(
      `/connect-digilocker?error=${encodeURIComponent(error.message)}`,
    );
  }
});
...
router.get("/digilocker/documents", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_LIST_FAILED" });
  }
});
...
router.post("/digilocker/import", authMiddleware, async (req, res) => {
  try {
    ...
    const { document } = await digilockerService.pullDocument(
      userId,
      documentUri,
    );
    ...
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_IMPORT_FAILED" });
  }
});
...
router.post("/digilocker/import-all", authMiddleware, async (req, res) => {
  try {
    ...
    // ⚡ Bolt: Parallelize DigiLocker document pulls for ~O(1) network time instead of sequential O(n).
    const pullPromises = documents.map(async (doc: any) => {
      try {
        const { document } = await digilockerService.pullDocument(
          userId,
          doc.uri,
        ...
        return { success: false, docName: doc.name };
      }
    });

    const results = await Promise.all(pullPromises);

    const credentialsToStore = results
      .filter((r) => r.success)
      .map((r) => r.credential as any);
    const successDocs = results
      .filter((r) => r.success)
      .map((r) => r.docName as string);
    failed.push(
      ...results.filter((r) => !r.success).map((r) => r.docName as string),
    );

    if (credentialsToStore.length > 0) {
      // ⚡ Bolt: Use batched persistence to avoid O(n) individual save queue requests
    ...
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_IMPORT_ALL_FAILED" });
  }
});
...
router.post("/digilocker/disconnect", authMiddleware, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_DISCONNECT_FAILED" });
  }
});
  ...
  try {
    ...
    if (digilockerService.isDemoMode()) {
      if (!allowDemoRoutes) {
        return res
          .status(404)
          .json({ error: "Not found", code: "DEMO_ROUTES_DISABLED" });
      }
      ...
      // ⚡ Bolt: Parallelize demo imports for improved connection speed
      const pullPromises = documents.slice(0, 3).map(async (doc: any) => {
        try {
          const { document } = await digilockerService.pullDocument(
            userId,
            doc.uri,
          ...
          return {
            ...
            credential: {
              ...
              category: doc.doctype.includes("CLASS")
                ? "academic"
                : "government",
            },
          };
    ...
    res
      .status(500)
      .json({ error: error.message, code: "DIGILOCKER_CONNECT_FAILED" });
  }
});
