...
const oauthStateStore = new Map<
  string,
  { userId: number; platform: string; expiresAt: number }
>();
...
async function listConnectionsForUser(
  userId: number,
...
async function listPendingRequestsForUser(
  userId: number,
  ...
  try {
    ...
    res.json({
      ...
      stats: {
        ...
        active: activeCount,
        totalAccessCount: connections.reduce(
          (sum, connection) => sum + connection.accessCount,
          0,
        ),
      },
    });
...
router.get("/oauth/:platform/callback", async (req: Request, res: Response) => {
  ...
  if (oauthError) {
    return res
      .status(400)
      .json({ success: false, error: "oauth_denied", reason: oauthError });
  }
  ...
  if (
    ...
    stateEntry.platform !== platform
  ) {
    return res
      .status(400)
      .json({ success: false, error: "invalid_or_expired_state" });
  }
  ...
  if (!config || !clientId || !clientSecret) {
    return res
      .status(503)
      .json({ success: false, error: "platform_not_configured" });
  }
  ...
  try {
    ...
    if (!tokenResp.ok) {
      ...
      return res
        .status(502)
        .json({ success: false, error: "token_exchange_failed" });
    }
    ...
    const encryptedRefresh = tokenData.refresh_token
      ? encrypt(tokenData.refresh_token, encKey)
      : null;
    ...
    if (db) {
      ...
      const existing = await db
        ...
        .where(
          and(
            eq(platformConnections.userId, stateEntry.userId),
            eq(platformConnections.platformId, platform),
          ),
        )
        .limit(1);
    ...
    try {
      ...
      const activity = connection.lastSyncedAt
        ? [
            {
              type: "sync",
              timestamp: connection.lastSyncedAt,
              description: "Connection metadata synced",
            },
          ]
        : [];
