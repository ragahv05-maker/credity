...
import {
  ...
  type ReputationScoreContract,
  type SafeDateScoreContract,
} from "@credverse/trust";
...
function normalizeReasonCode(code: string): ReasonCode {
  return code
    .trim()
    .toUpperCase()
    .replace(/[^A-Z0-9]+/g, "_") as ReasonCode;
}

function isTrustSdkEnabled(): boolean {
  return (
    String(process.env.REPUTATION_TRUST_SDK_ENABLED || "").toLowerCase() ===
    "true"
  );
}
...
function mapVerificationDecision(
  score: number,
  safeDateScore: number,
  reasonCodes: string[],
): CandidateVerificationSummary["decision"] {
  const normalized = reasonCodes.map(normalizeReasonCode);
  if (
    normalized.includes("HARASSMENT_REPORTS_PRESENT") ||
    normalized.includes("BACKGROUND_FLAGS_PRESENT")
  ) {
    return "investigate";
  }
...
async function buildCandidateVerificationSummary(
  userId: number,
  ...
  const confidence = Math.max(
    0.5,
    Math.min(0.99, Number((reputationScore.score / 1000).toFixed(2))),
  );
...
async function buildSafeDateSnapshot(
  userId: number,
): Promise<{
  reputationScore: ReputationScoreContract;
  safeDate: SafeDateScoreContract;
}> {
  ...
  if (trustSdk) {
    try {
      ...
      console.warn(
        "[reputation] Trust SDK fetch failed, using local fallback:",
  ...
  const documentStatus = documentService.getDocumentVerificationStatus(
    String(userId),
  );
...
router.get("/events", async (req: Request, res: Response) => {
  try {
    ...
    const category =
      categoryRaw &&
      ALLOWED_CATEGORIES.includes(categoryRaw as ReputationCategory)
        ? (categoryRaw as ReputationCategory)
        : undefined;
    const limit = Math.max(1, Math.min(500, Number(req.query.limit) || 100));

    const events = (await listReputationEvents(userId, category)).slice(
      0,
      limit,
    );
    return res.json({
...
router.post("/events", async (req: Request, res: Response) => {
  try {
    ...
    const { reputationScore, safeDate } = await buildSafeDateSnapshot(
...
router.get("/summary", async (req: Request, res: Response) => {
  try {
    ...
    return res.status(400).json({
      success: false,
      error:
        error?.message || "Failed to calculate candidate verification summary",
    });
  }
});
