...
const claimVerifySchema = z.object({
  ...
  evidence: z
    ...
    .default([]),
  user_credentials: z
    .array(z.object({ type: z.string(), issuer: z.string().optional() }))
    .optional(),
  userCredentials: z
    .array(z.object({ type: z.string(), issuer: z.string().optional() }))
    .optional(),
...
router.patch("/:id/status", async (req: Request, res: Response) => {
  try {
    ...
    res
      .status(500)
      .json({ success: false, error: "Failed to update claim status" });
  }
});
...
router.get("/", async (req: Request, res: Response) => {
  try {
    ...
    const stats = {
      ...
      investigate: claims.filter((c) => c.recommendation === "investigate")
        .length,
      rejected: claims.filter((c) => c.recommendation === "reject").length,
      avgTrustScore:
        claims.length > 0
          ? Math.round(
              claims.reduce((sum, c) => sum + c.trustScore, 0) / claims.length,
            )
          : 0,
    };
