...
const proofPayloadSchema = z
  ...
  .transform((value) => ({
    ...
    algorithm:
      value.algorithm ||
      (value.hashAlgorithm === "sha256" ? "sha256" : undefined),
...
router.get("/wallet/credentials", authMiddleware, async (req, res) => {
  try {
    ...
    let credentials;
    if (category) {
      credentials = await walletService.getCredentialsByCategory(
        userId,
        category,
      );
    } else {
...
router.post("/wallet/offer/claim", authMiddleware, async (req, res) => {
  try {
    ...
    const proofMeta = parsedProof.success
      ? sanitizeUnsafeMetadata(parsedProof.data)
      : null;
    ...
    const decodedPayload =
      typeof vcJwt === "string" && vcJwt.length > 0
        ? decodeJwtPayload(vcJwt)
        : null;
    ...
    const credType =
      credData.credentialData?.credentialName ||
    ...
    res
      .status(500)
      .json({ error: "Failed to claim offer", code: "OFFER_CLAIM_FAILED" });
  }
});
