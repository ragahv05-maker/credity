...
router.get("/suggestions", async (req: Request, res: Response) => {
  try {
    ...
    const quickWins = suggestions.filter(
      (s: ImprovementSuggestion) => s.category === "quick_win",
    );
    const longTerm = suggestions.filter(
      (s: ImprovementSuggestion) => s.category === "long_term",
    );
    ...
    res.json({
      ...
      quickWins,
      longTerm,
      potentialPoints: suggestions.reduce(
        (sum: number, s: ImprovementSuggestion) => sum + s.points,
        0,
      ),
    });
  } catch (error: any) {
...
async function getUserTrustData(userId: number): Promise<UserTrustData> {
  ...
  const livenessStatus = livenessService.getUserLivenessStatus(
    userId.toString(),
  );
  const biometricsStatus = biometricsService.getBiometricStatus(
    userId.toString(),
  );
  ...
  if (db) {
    // Batch independent db queries for performance
    const [activityResult, connResult, endResult, posResult, negResult] =
      await Promise.all([
        ...
        db
          ...
          .where(
            and(
              eq(reputationEvents.userId, userId),
              lt(reputationEvents.score, 50),
            ),
          ),
      ]);

    totalVerifications = Number(activityResult[0]?.value ?? 0);
    platformConnectionCount = Number(connResult[0]?.value ?? 0);
    endorsementCount = Number(endResult[0]?.value ?? 0);
    positiveFeedbackCount = Number(posResult[0]?.value ?? 0);
    negativeFeedbackCount = Number(negResult[0]?.value ?? 0);
  }
