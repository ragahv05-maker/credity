export const ReasonCodes = {
  ...
  INTEGRITY_CLAIM_AMOUNT_UNUSUALLY_HIGH:
    "INTEGRITY_CLAIM_AMOUNT_UNUSUALLY_HIGH",
  INTEGRITY_REFUND_AMOUNT_ABOVE_THRESHOLD:
    "INTEGRITY_REFUND_AMOUNT_ABOVE_THRESHOLD",

  // Evidence (PROOF)
  EVIDENCE_NONE_PROVIDED: "EVIDENCE_NONE_PROVIDED",
  EVIDENCE_DEEPFAKE_DETECTED: "EVIDENCE_DEEPFAKE_DETECTED",
  EVIDENCE_DEEPFAKE_PROVIDER_UNAVAILABLE:
    "EVIDENCE_DEEPFAKE_PROVIDER_UNAVAILABLE",
...
export function stableSortReasonCodes(codes: Iterable<string>): string[] {
  return Array.from(new Set(Array.from(codes).filter(Boolean))).sort((a, b) =>
    a.localeCompare(b),
  );
}
