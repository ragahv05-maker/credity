...
async function calculateAndPersistScore(userId: number): Promise<void> {
  ...
  await storage.upsertSafeDateScore(userId, {
    ...
    trustLevel:
      safeDateScore.score >= 70
        ? "high"
        : safeDateScore.score >= 45
          ? "medium"
          : "low",
...
function computeReputationScore(
  ...
  const breakdown: ReputationCategoryBreakdown[] = CATEGORY_LIST.map(
    (category) => {
  ...
  const score100 = breakdown.reduce(
    (sum, entry) => sum + entry.weighted_score,
    0,
  );
...
function computeSafeDateInputs(
  events: ReputationEventRecord[],
): Omit<SafeDateInputs, "identityVerified" | "livenessVerified"> {
...
export async function upsertReputationEvent(
  input: ReputationEventInput,
  ...
  if (existingStored) {
    return {
      accepted: false,
      duplicate: true,
      event: toEventRecord(existingStored),
    };
  }
...
export async function calculateReputationScore(
  userId: number,
): Promise<ReputationScoreSnapshot> {
...
export async function deriveSafeDateInputs(
  userId: number,
): Promise<Omit<SafeDateInputs, "identityVerified" | "livenessVerified">> {
  const events = await listReputationEvents(userId);
...
export function calculateSafeDateScore(
  ...
  const backgroundPoints =
    inputs.backgroundFlags === 0
      ? 20
      : Math.max(0, 20 - Math.min(inputs.backgroundFlags * 10, 20));
  ...
  const harassmentFreePoints =
    inputs.harassmentReports === 0
      ? 10
      : Math.max(0, 10 - Math.min(inputs.harassmentReports * 5, 10));
  ...
  if (inputs.harassmentReports > 0)
    reasonCodes.push("harassment_reports_present");
