...
const DIGILOCKER_CONFIG = {
  ...
  redirectUri:
    ...
    "http://localhost:5002/api/digilocker/callback",
...
export class DigiLockerService {
  ...
  constructor() {
    ...
    this.clientId = this.isConfigured
      ? configuredClientId
      : this.demoMode
        ? "demo_client_id"
        : "";
    this.clientSecret = this.isConfigured
    ...
    if (!this.isConfigured && !this.demoMode) {
      console.warn(
        "[DigiLocker] Credentials are not configured. DigiLocker operations are disabled.",
      );
    }
  }
