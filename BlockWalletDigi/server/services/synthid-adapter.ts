export type SynthIdVerdict = 'watermark_detected' | 'not_detected' | 'unsupported' | 'unavailable' | 'error';
...
export async function assessSynthIdWatermark(
  ...
  if (input.mediaType === 'document') {
    return {
      ...
      confidence: null,
      provider: 'fallback',
      reason: 'SynthID watermark checks apply to AI-generated media, not scanned documents.',
    };
  }
  ...
  try {
    ...
    if (verdict !== 'watermark_detected' && verdict !== 'not_detected' && verdict !== 'unsupported') {
      return {
        version: 'synthid-v1',
        verdict: 'error',
        confidence: null,
