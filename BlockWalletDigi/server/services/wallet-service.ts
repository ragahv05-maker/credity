...
type PersistedStoredCredential = Omit<
  StoredCredential,
  "issuanceDate" | "expirationDate" | "lastVerified"
> & {
  issuanceDate: string;
...
type PersistedShareRecord = Omit<
  ShareRecord,
  "expiry" | "createdAt" | "accessLog"
> & {
  expiry: string;
  createdAt: string;
...
type PersistedConsentLog = Omit<ConsentLog, "timestamp"> & {
  timestamp: string;
};
type PersistedNotification = Omit<WalletNotification, "timestamp"> & {
  timestamp: string;
};

type PersistedWalletState = Omit<
  WalletState,
  "lastSync" | "credentials" | "shares" | "consentLogs" | "notifications"
> & {
  lastSync: string;
  ...
  return {
    ...
    credentials: wallet.credentials.map((credential) => ({
      ...
      lastVerified: credential.lastVerified
        ? new Date(credential.lastVerified)
        : undefined,
    })),
    shares: wallet.shares.map((share) => ({
      ...share,
...
export class WalletService {
  ...
  async storeCredential(
    ...
    this.addNotification(userId, {
      ...
      data: {
        ...
        txHash: storedCredential.anchorTxHash,
      },
    });
  ...
  async accessShare(
    ...
    for (const [userId, wallet] of Array.from(wallets.entries())) {
      const share = wallet.shares.find(
        (s: ShareRecord) => s.id === shareId || s.token === shareId,
      );
      ...
      if (share) {
        ...
        const credential = wallet.credentials.find(
          (c: StoredCredential) => c.id === share.credentialId,
        );
        if (!credential) {
        ...
        const disclosedData = this.applySelectiveDisclosure(
          credential.data,
          share.disclosedFields,
        );
   ...
   */
  async getCredentialsByCategory(
    userId: number,
    category: string,
  ): Promise<StoredCredential[]> {
    const wallet = await this.getOrCreateWallet(userId);
  ...
  async revokeConsentGrant(
    userId: number,
    consentGrantId: string,
  ): Promise<ConsentGrantRecord | null> {
    const wallet = await this.getOrCreateWallet(userId);
    const consent = wallet.consentGrants.find(
      (item) => item.id === consentGrantId,
    );
    if (!consent) {
      return null;
    }
  ...
  async listCertInIncidents(): Promise<
    Array<CertInIncidentRecord & { seconds_to_report_due: number }>
  > {
    await ensureHydrated();
    const now = Date.now();
  ...
  async getNotifications(userId: number): Promise<WalletNotification[]> {
    const wallet = await this.getOrCreateWallet(userId);
    return wallet.notifications.sort(
  ...
  async markNotificationRead(
    ...
    const wallet = await this.getOrCreateWallet(userId);
    const notification = wallet.notifications.find(
  ...
  async createBackup(
    userId: number,
  ): Promise<{ backupData: string; backupKey: string }> {
    const wallet = await this.getOrCreateWallet(userId);
    const backupKey = crypto.randomBytes(32).toString("hex");

    const backupData = this.encrypt(
      JSON.stringify({
  ...
  async getWalletStats(userId: number): Promise<{
    ...
    const activeShares = wallet.shares.filter(
      (s) => !s.revoked && new Date() < s.expiry,
    ).length;
    const totalVerifications = wallet.shares.reduce(
  ...
  private async anchorCredential(
    ...
    if (anchorServiceUrl) {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Credential anchoring failed: ${response.status} ${errorBody}`,
        );
      }
      ...
      if (!data.txHash || typeof data.txHash !== "string") {
        throw new Error(
          "Credential anchoring failed: anchor service response missing txHash",
        );
      }
      ...
      return {
        ...
        blockNumber:
          ...
          Number.isFinite(data.blockNumber)
