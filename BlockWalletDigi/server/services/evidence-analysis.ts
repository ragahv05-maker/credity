...
import {
  assessSynthIdWatermark,
  type SynthIdAssessment,
} from "./synthid-adapter";
...
async function anchorToPolygon(proofMetadataHash: string): Promise<{
  ...
  try {
    ...
    console.info(
      `[Evidence] Polygon anchor queued for hash ${proofMetadataHash} — install viem for full submission`,
    );
    return { status: "pending", chain: "polygon-amoy" };
  } catch {
    return { status: "failed", chain: "polygon-amoy" };
...
export async function analyzeEvidence(
  ...
  const { isAiGenerated, unknownConfidencePenalty } =
    await checkAiGenerated(request);
...
async function checkAiGenerated(request: EvidenceUploadRequest): Promise<{
  ...
  const penalty =
    result.provider === "not_configured" || result.provider === "validation"
      ? 0
      : 10;
