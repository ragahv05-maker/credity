...
const validators: Record<SupportedDocType, (value: string) => boolean> = {
  aadhaar: (value) => {
    ...
    return (
      /^[2-9][0-9]{11}$/.test(normalized) && !isLikelyFakeAadhaar(normalized)
    );
  },
  pan: (value) =>
    /^[A-Z]{5}[0-9]{4}[A-Z]$/.test(normalizeDocumentNumber(value)),
  passport: (value) =>
    /^[A-PR-WY][0-9]{7}$/.test(normalizeDocumentNumber(value)),
  driving_license: (value) =>
    /^[A-Z]{2}[0-9]{2}[0-9]{11,13}$/.test(normalizeDocumentNumber(value)),
};
...
export function validateDocumentByType(
  ...
  if (!documentNumber || typeof documentNumber !== "string") {
    return {
      valid: false,
      normalizedType,
      reason: "documentNumber is required",
    };
  }
