...
function parseJwksPayload(payload: unknown): AppleSigningJwk[] {
    ...
    return (payload as { keys: unknown[] }).keys
        .filter((key): key is AppleSigningJwk => {
...
async function fetchAppleJwks(forceRefresh = false): Promise<AppleSigningJwk[]> {
    const now = Date.now();
...
function findJwkByKid(keys: AppleSigningJwk[], kid: string): AppleSigningJwk | null {
    return keys.find((key) => key.kid === kid && (!key.use || key.use === 'sig')) ?? null;
}
...
export async function verifyAppleIdentityToken(identityToken: string): Promise<AppleIdentityPayload> {
