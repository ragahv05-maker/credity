...
function resolveTokenEncryptionKey(): string {
  ...
  return crypto
    .createHash("sha256")
    .update(
      process.env.DIGILOCKER_CLIENT_SECRET || "wallet-digilocker-user-pull-dev",
    )
    .digest("hex");
}
...
async function persistUserPullState(): Promise<void> {
  ...
  userPullPersistChain = userPullPersistChain
    ...
    .catch((error) => {
      console.error(
        "[Wallet DigiLocker] Failed to persist user-pull token state:",
        error,
      );
    });
...
export async function getStoredUserPullTokens(
  userId: number,
): Promise<UserPullTokenBundle | null> {
  await ensureUserPullStateHydrated();
  ...
  try {
    ...
    console.error(
      "[Wallet DigiLocker] Failed to decrypt stored user-pull tokens:",
      error,
    );
    return null;
  }
