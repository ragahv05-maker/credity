...
export function matchFace(input: FaceMatchInput): {
  ...
  const idEmbedding =
    input.idFaceEmbedding ??
    (input.idImageData ? embeddingFromImageData(input.idImageData) : null);
  const liveEmbedding =
    input.liveFaceEmbedding ??
    (input.liveImageData ? embeddingFromImageData(input.liveImageData) : null);

  if (!idEmbedding || !liveEmbedding) {
    throw new Error(
      "Either embeddings or image data for both faces are required",
    );
  }
  ...
  const confidence = Number(
    cosineSimilarity(idEmbedding, liveEmbedding).toFixed(4),
  );
  return {
    confidence,
