...
function assertApiConfigured(key: string, serviceName: string): boolean {
  if (!process.env[key]) {
    if (isProduction) {
      throw new Error(
        `${serviceName} API key (${key}) is required in production`,
      );
    }
...
function detectDocumentTypeFromText(textBlocks: string[]): string {
  ...
  if (
    fullText.includes("AADHAAR") ||
    fullText.includes("आधार") ||
    fullText.includes("UIDAI")
  ) {
    return "aadhaar";
  }
  if (
    fullText.includes("INCOME TAX") ||
    fullText.includes("PAN") ||
    fullText.includes("PERMANENT ACCOUNT NUMBER")
  ) {
    return "pan";
  }
  if (
    fullText.includes("PASSPORT") ||
    fullText.includes("REPUBLIC OF INDIA") ||
    fullText.includes("TRAVEL DOCUMENT")
  ) {
    return "passport";
  }
  if (
    fullText.includes("DRIVING LICENCE") ||
    fullText.includes("DRIVING LICENSE") ||
  ...
  if (
    ...
    fullText.includes("VOTER") ||
    fullText.includes("ELECTION COMMISSION")
  ) {
    return "voter_id";
  }
...
function extractFieldsFromOcrText(
  ...
  if (nameMatch) {
    ...
    const nameLabelMatch = fullText.match(
      /(?:Name|नाम)\s*[:\-]?\s*([A-Za-z ]{2,40})/i,
    );
    if (nameLabelMatch) {
      fields.push({
        field: "name",
        value: nameLabelMatch[1].trim(),
        confidence: 0.7,
      });
    }
  }
  ...
  const genderMatch = fullText.match(
    /\b(Male|Female|MALE|FEMALE|पुरुष|महिला|Transgender)\b/i,
  );
  if (genderMatch) {
    fields.push({
      field: "gender",
      value: genderMatch[1].trim(),
      confidence: 0.98,
    });
  }
  ...
  if (documentType === "aadhaar" || documentType === "driving_license") {
    const addressMatch = fullText.match(
      /(?:Address|पता)\s*[:\-]?\s*([\s\S]{10,120}?)(?:\n\n|\d{6})/i,
    );
    if (addressMatch) {
      fields.push({
        field: "address",
        value: addressMatch[1].replace(/\n/g, ", ").trim(),
        confidence: 0.7,
      });
    }
  }
  ...
  if (documentType === "pan") {
    const fatherMatch = fullText.match(
      /(?:Father|पिता)\s*(?:'s)?\s*(?:Name)?\s*[:\-]?\s*([A-Z ]{2,40})/i,
    );
    if (fatherMatch) {
      fields.push({
        field: "father_name",
  ...
  const expiryMatch = fullText.match(
    /(?:Valid|Expiry|Validity|Valid\s*(?:Till|Upto|Until))\s*[:\-]?\s*(\d{2}[\/\-]\d{2}[\/\-]\d{4}|\d{4}-\d{2}-\d{2})/i,
  );
  if (expiryMatch) {
    fields.push({
      field: "expiry_date",
  ...
  const issueMatch = fullText.match(
    /(?:Issue\s*Date|Date\s*of\s*Issue)\s*[:\-]?\s*(\d{2}[\/\-]\d{2}[\/\-]\d{4}|\d{4}-\d{2}-\d{2})/i,
  );
  if (issueMatch) {
    fields.push({
      field: "issue_date",
  ...
  switch (documentType) {
    case "aadhaar":
      fields.push(
        ...
        {
          field: "address",
          value: "House 123, Sector 15, Gurgaon, Haryana 122001",
          confidence: 0.85,
        },
      );
      break;
...
export async function scanDocument(
  ...
  const documentType =
    request.documentType === "auto" || !request.documentType
      ? hasOcr
        ? detectDocumentTypeFromText(textBlocks)
        : "aadhaar"
  ...
  const authenticityChecks = performAuthenticityChecks(
    request.imageData,
    documentType,
    textBlocks,
    extractedFields,
  ...
  const overallScore = calculateDocumentScore(
    extractedFields,
    authenticityChecks,
  );
...
function parseExtractedData(
  ...
  documentType: string,
): DocumentScanResult["extractedData"] {
 ...
 */
function extractFaceFromDocument(imageData: string): {
  found: boolean;
  imageData?: string;
} {
...
function performAuthenticityChecks(
  ...
  const docFormatPassed =
    docNumField && docNumPattern
...
function calculateDocumentScore(
  fields: ExtractedField[],
  checks: DocumentScanResult["authenticityChecks"],
): number {
  ...
  const avgCheckConfidence =
    passedChecks.reduce((sum, c) => sum + c.confidence, 0) / checks.length;
  ...
  // Weighted score
  const score =
    avgFieldConfidence * 40 + avgCheckConfidence * 30 + passRate * 30;
...
function generateWarnings(
  data: DocumentScanResult["extractedData"],
  checks: DocumentScanResult["authenticityChecks"],
): string[] {
