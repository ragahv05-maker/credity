...
if (missingJwtSecrets && !requireStrictSecrets) {
  console.warn(
    "WARNING: Using development JWT secrets. Set JWT_SECRET and JWT_REFRESH_SECRET for staging/production.",
  );
}
...
export function generateRefreshToken(
  ...
  const token = jwt.sign(payload, EFFECTIVE_JWT_REFRESH_SECRET, {
...
export function __test_backdateRefreshSession(
  refreshToken: string,
  sessionStartedAt: Date,
): boolean {
  const record = refreshTokens.get(refreshToken);
