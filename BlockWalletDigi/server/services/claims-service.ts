...
export async function verifyClaim(
  ...
  const riskSignals = [
    ...identityResult.riskSignals,
    ...integrityResult.riskSignals,
    ...authenticityResult.riskSignals,
  ]
    .map((signal) => ({
      ...signal,
      reason_codes: stableSortReasonCodes(signal.reason_codes) as ReasonCode[],
    }))
...
async function verifyIdentity(
  ...
  if (
    request.userCredentials.includes("age_18") ||
    request.userCredentials.includes("age_21")
  ) {
    score += 15;
  ...
  riskSignals.push({
    ...
    details: {
      ...
      provided_count: request.userCredentials.length,
    },
  });
...
async function validateClaim(
  ...
  if (fraudPatternMatch > 0.7) {
    score -= 30;
    redFlags.push(
      `High fraud pattern match: ${(fraudPatternMatch * 100).toFixed(0)}%`,
    );
  ...
  if (llmConfidence > 0.8) {
    ...
    score -= 10;
    redFlags.push(
      `Claim description raises concerns (${confidenceResult.reason})`,
    );
  ...
  if (request.claimAmount) {
    if (
      request.claimType === "insurance_auto" &&
      request.claimAmount > 1000000
    ) {
  ...
  riskSignals.push({
    ...
    details: {
      ...
      issues_count: timelineAnalysis.issues.length,
    },
  });
  ...
  riskSignals.push({
    ...
    details: {
      ...
      provider: confidenceResult.provider,
      provider_reason: confidenceResult.reason,
    },
  });
...
async function authenticateEvidence(
  ...
  const mediaEvidence = request.evidence.filter(
    (item) => item.type === "image" || item.type === "video",
  );
  const deepfakeResults = await Promise.all(
    mediaEvidence.map((item) => detectDeepfakeFromUrl(item.url)),
  );
  const hasFake = deepfakeResults.some((result) => result.verdict === "fake");
  const hasUnknown =
    deepfakeResults.length > 0 &&
    deepfakeResults.every((result) => result.verdict === "unknown");
  ...
  if (deepfakeDetected) {
    ...
    score -= 10;
    redFlags.push(
      "Deepfake detection provider unavailable; authenticity confidence reduced",
    );
  ...
  riskSignals.push({
    ...
    score: deepfakeRisk,
    severity:
      deepfakeRisk >= 1 ? "high" : deepfakeRisk >= 0.4 ? "medium" : "info",
  ...
  return {
    ...
    riskSignals,
    deepfakeDetected,
    deepfakeVerdict: deepfakeDetected
      ? "fake"
      : hasUnknown
        ? "unknown"
        : "real",
 ...
 */
function analyzeTimeline(timeline: TimelineEvent[]): {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    const timeDiffMs =
      new Date(curr.time).getTime() - new Date(prev.time).getTime();
    ...
    if (prev.location !== curr.location && timeDiffMinutes < 30) {
      ...
      if (!isSameCity(prev.location, curr.location)) {
        issues.push(
          `Impossible travel: ${prev.location} to ${curr.location} in ${timeDiffMinutes} minutes`,
        );
      }
    }
...
function getRecommendation(
  trustScore: number,
): ClaimVerifyResponse["recommendation"] {
...
function calculateCost(
  request: ClaimVerifyRequest,
): ClaimVerifyResponse["costBreakdown"] {
  const identityVerification = 0; // Free (already verified in wallet)
  const mlInference = 2.0; // Base ML cost
  const llmAnalysis = 0.02; // ~500 tokens at ₹0.04/1K
  const deepfakeConfigured = Boolean(
    process.env.DEEPFAKE_API_URL && process.env.DEEPFAKE_API_KEY,
  );
  const deepfakeCheck =
    request.evidence.length > 0 && deepfakeConfigured ? 0.5 : 0;
...
export async function getClaimById(
  claimId: string,
): Promise<ClaimVerifyResponse | null> {
