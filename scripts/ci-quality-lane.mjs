...
const mode = (modeArg?.split('=')[1] || process.env.CI_QUALITY_MODE || 'full').toLowerCase();
...
const run = (label, command) => {
  ...
  try {
    ...
    return { label, command, status: 'pass', durationSec: ((Date.now() - startedAt) / 1000).toFixed(1) };
  } catch (error) {
    return { label, command, status: 'fail', durationSec: ((Date.now() - startedAt) / 1000).toFixed(1), code: error?.status ?? 1 };
  }
};
...
const summary = [
  ...
  '## Results',
  ...results.map(
    (result) => `- ${result.status === 'pass' ? '✅' : '❌'} ${result.label} (${result.durationSec}s) — \`${result.command}\``
  ),
  ''
].join('\n');
