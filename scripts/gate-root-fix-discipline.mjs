...
const codeFilePattern =
  /^(BlockWalletDigi|CredVerseRecruiter|CredVerseIssuer 3|packages\/trust-sdk)\/(server|src|client|shared)\//;
const testFilePattern =
  /^(BlockWalletDigi|CredVerseRecruiter|CredVerseIssuer 3|packages\/trust-sdk)\/tests\/.*\.(test|spec)\./;
...
if (changedCodeFiles.length > 0 && changedTestFiles.length === 0) {
  console.error(
    "❌ Root-fix gate failed: code changed but no test files updated.",
  );
  console.error(
    "   Add or update at least one relevant test before claiming completion.",
  );
  process.exit(1);
}
...
for (const file of changedFiles) {
  ...
  if (forbiddenNameMarkers.some((marker) => lower.includes(marker))) {
    console.error(
      `❌ Root-fix gate failed: suspicious redundant file name detected -> ${file}`,
    );
    process.exit(1);
  }
}
...
for (const file of changedCodeFiles) {
  ...
  for (const marker of forbiddenContentMarkers) {
    if (content.toLowerCase().includes(marker.toLowerCase())) {
      console.error(
        `❌ Root-fix gate failed: found forbidden marker "${marker}" in ${file}`,
      );
      process.exit(1);
    }
  }
