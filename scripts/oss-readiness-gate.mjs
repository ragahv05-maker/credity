...
const run = (label, command, cwd = root) => {
  ...
  try {
    ...
    rows.push({ label, command, status: 'pass', durationSec: ((Date.now() - startedAt) / 1000).toFixed(1) });
  } catch (error) {
    rows.push({ label, command, status: 'fail', durationSec: ((Date.now() - startedAt) / 1000).toFixed(1), code: error?.status ?? 1 });
    throw error;
  }
};
...
try {
  ...
  const summary = [
    ...
    '',
    '## Results',
    ...rows.map((r) => `- ${r.status === 'pass' ? '✅' : r.status === 'skip' ? '⏭️' : '❌'} ${r.label} (${r.durationSec}s) — \`${r.command}\``),
    ''
  ].join('\n');
  fs.writeFileSync(path.join(evidenceDir, 'oss-readiness-summary.md'), summary, 'utf8');
}
