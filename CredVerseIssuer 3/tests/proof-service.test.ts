...
} from "../server/services/proof-service";
...
describe("issuer proof-service", () => {
  it("generates executable merkle-membership proof envelope", () => {
    const result = generateProof({
      ...
      credential: {
        ...
        credentialData: {
          credentialSubject: { id: "did:key:subject-1", degree: "B.Tech" },
        },
      },
    ...
    expect((result.proof as any).verification_contract).toBe(
      "credity-proof-verification/v1",
    );
    expect((result.proof as any).leaf_hash).toBeTypeOf("string");
    expect((result.public_signals as any).leaf_hash).toBe(
      (result.proof as any).leaf_hash,
    );
  });
  ...
  it("keeps signing source-of-truth strict when generating claims digest", () => {
    ...
    const legacyDigest = deterministicHashLegacyTopLevel(
      credentialData,
      "sha256",
    );
  ...
  it("verifies valid merkle-membership proof deterministically", () => {
    const credentialData = {
      credentialSubject: { id: "did:key:subject-1", score: 93 },
    };
