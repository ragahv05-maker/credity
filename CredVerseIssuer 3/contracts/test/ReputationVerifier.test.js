...
describe("ReputationVerifier", function () {
  ...
  beforeEach(async function () {
    ...
    await Promise.all([
      mock1.waitForDeployment(),
      mock2.waitForDeployment(),
      mock3.waitForDeployment(),
    ]);
  ...
  it("stores proof hash when verifier accepts proof", async function () {
    ...
    const event = receipt.logs.find(
      (l) => l.fragment && l.fragment.name === "ProofVerified",
    );
  ...
  it("reverts on duplicate proof submission", async function () {
    ...
    await expect(
      verifier.verifyAndStoreProof(...args),
    ).to.be.revertedWithCustomError(verifier, "ProofAlreadyStored");
  });
  ...
  it("routes verification by circuit id and allows admin verifier rotation", async function () {
    ...
    await expect(
      verifier.setCircuitVerifier(1, await newMock.getAddress()),
    ).to.emit(verifier, "ZkVerifierUpdated");

    expect(await verifier.zkVerifierByCircuit(1)).to.equal(
  ...
  it("blocks non-admin verifier rotation", async function () {
    await expect(
      verifier.connect(other).setCircuitVerifier(1, await mock1.getAddress()),
    ).to.be.reverted;
  });
