...
async function resolveApiKeyTenant(
  keyHeader: string,
): Promise<{ tenantId: string; keyHash: string; error?: string }> {
...
function enforceApiRateLimit(keyHash: string): {
  allowed: boolean;
  error?: string;
} {
  const now = Date.now();
...
export async function apiKeyMiddleware(
  ...
  if (resolved.error) {
    return res
      .status(401)
      .json({ message: resolved.error, code: "AUTH_UNAUTHORIZED" });
  }
  ...
  if (!rate.allowed) {
    return res
      .status(429)
      .json({ message: rate.error, code: "AUTH_RATE_LIMITED" });
  }
...
export async function apiKeyOrAuthMiddleware(
  ...
  if (!payload) {
    return res
      .status(401)
      .json({ message: "Invalid or expired token", code: "AUTH_UNAUTHORIZED" });
  }

  (req as any).user = payload;
  const tokenTenantId =
    typeof (payload as any).tenantId === "string" &&
    (payload as any).tenantId.trim().length > 0
