...
type QueueWorkflowHook = (
  event: QueueWorkflowHookEvent,
) => void | Promise<void>;

let queueWorkflowHook: QueueWorkflowHook | null = null;

async function emitQueueWorkflowHook(
  event: QueueWorkflowHookEvent,
): Promise<void> {
...
export function registerQueueWorkflowHook(
  hook: QueueWorkflowHook | null,
): void {
  queueWorkflowHook = hook;
}

const DEFAULT_JOB_ATTEMPTS = Number(process.env.ISSUANCE_JOB_ATTEMPTS || 3);
const DEFAULT_BACKOFF_DELAY_MS = Number(
  process.env.ISSUANCE_BACKOFF_DELAY_MS || 1000,
);
const DEFAULT_BACKOFF_MAX_DELAY_MS = Number(
  process.env.ISSUANCE_BACKOFF_MAX_DELAY_MS || 30_000,
);
...
function resolveRetryConfig(): {
  ...
  const attempts =
    Number.isFinite(DEFAULT_JOB_ATTEMPTS) && DEFAULT_JOB_ATTEMPTS > 0
      ? Math.floor(DEFAULT_JOB_ATTEMPTS)
      : 3;
  const delayMs =
    Number.isFinite(DEFAULT_BACKOFF_DELAY_MS) && DEFAULT_BACKOFF_DELAY_MS > 0
      ? Math.floor(DEFAULT_BACKOFF_DELAY_MS)
      : 1000;
  const maxDelayMs =
    Number.isFinite(DEFAULT_BACKOFF_MAX_DELAY_MS) &&
    DEFAULT_BACKOFF_MAX_DELAY_MS >= delayMs
  ...
  persistChain = persistChain
    .then(async () => {
      await stateStore.save({
        jobResults: Array.from(jobResults.entries()).map(([jobId, result]) => [
          jobId,
...
export async function initQueueService(): Promise<boolean> {
  ...
  const requireQueue =
    ...
    process.env.REQUIRE_QUEUE === "true";

  // Skip Redis if not configured
  if (!REDIS_URL) {
    if (requireQueue) {
      throw new Error(
        "[Queue] REDIS_URL is required by current runtime policy",
      );
    }
    console.log(
      "[Queue] REDIS_URL not configured; queue-backed bulk operations are disabled",
    );
    return false;
  }
  ...
  try {
    ...
    queueEvents.on("failed", (event) => {
      ...
      const attemptsMadeRaw = (event as { attemptsMade?: unknown })
        .attemptsMade;
      const attemptsMade =
        typeof attemptsMadeRaw === "number" && Number.isFinite(attemptsMadeRaw)
          ? attemptsMadeRaw
          : 0;
      console.warn(
        `[Queue] Event failed job=${jobId} attempts=${attemptsMade} prev=${prev} reason=${failedReason}`,
      );
    });
    ...
    console.log(
      `[Queue] Queue service initialized (attempts=${retryConfig.attempts}, backoff=${retryConfig.delayMs}ms exponential)`,
    );
    return true;
  } catch (error) {
    if (requireQueue) {
    ...
    console.warn(
      "[Queue] Redis not available; queue-backed bulk operations are disabled",
    );
...
async function addDeadLetterEntry(
...
export function startIssuanceWorker(
  ...
  issuanceWorker = new Worker(
    ...
    async (job: Job<IssuanceJobData>) => {
      ...
      console.log(
        `[Queue] Processing job ${jobId} with ${recipients.length} credentials`,
      );
      ...
      for (let i = 0; i < recipients.length; i++) {
        ...
        try {
          ...
          console.error(
            `[Queue] Job ${jobId} failed for recipient ${i + 1}:`,
            error,
          );
        }
      ...
      console.log(
        `[Queue] Job ${jobId} completed: ${result.success}/${result.total} success`,
      );
      void emitQueueWorkflowHook({
        type: "job.completed",
        jobId,
    ...
    if (job?.id && job.data) {
      ...
      if (result) {
        if (hasRetriesRemaining) {
          ...
          result.errors.push(
            `Attempt ${attemptsMade}/${configuredAttempts} failed: ${error.message}`,
          );
        } else {
          result.status = "failed";
          result.errors.push(
            `Job failed after ${attemptsMade} attempts: ${error.message}`,
          );
          void emitQueueWorkflowHook({
            type: "job.failed",
            jobId: sourceJobId,
      ...
      if (hasRetriesRemaining) {
        console.warn(
          `[Queue] Job ${sourceJobId} attempt ${attemptsMade}/${configuredAttempts} failed, retrying`,
        );
        return;
      }
      ...
      void addDeadLetterEntry(payload)
        .then((entryId) => {
          if (entryId) {
            console.warn(
              `[Queue] Job ${sourceJobId} moved to dead-letter queue entry ${entryId}`,
            );
            void emitQueueWorkflowHook({
            ...
            console.warn(
              `[Queue] Dead-letter queue unavailable, could not persist failed job ${sourceJobId}`,
            );
          }
        })
        .catch((dlqError) => {
          console.error(
            `[Queue] Failed to persist dead-letter entry for ${sourceJobId}:`,
            dlqError,
          );
        });
    }
 ...
 */
export async function addBulkIssuanceJob(
  data: IssuanceJobData,
  ...
  console.log(
    `[Queue] Bulk issuance job ${job.id} added with ${data.recipients.length} credentials`,
  );
  void emitQueueWorkflowHook({
    type: "job.queued",
    jobId: String(job.id),
...
export async function replayDeadLetterJob(
  entryId: string,
): Promise<{ deadLetterEntryId: string; replayJobId: string }> {
  await ensureHydrated();
