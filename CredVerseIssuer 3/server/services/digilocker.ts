...
class DigiLockerService {
  ...
  constructor() {
    ...
    this.config = {
      ...
      baseUrl:
        process.env.DIGILOCKER_BASE_URL || "https://api.digitallocker.gov.in",
      redirectUri:
        process.env.DIGILOCKER_REDIRECT_URI ||
        "http://localhost:5001/api/v1/digilocker/callback",
    };
    this.isConfigured = !!(this.config.clientId && this.config.clientSecret);

    if (!this.isConfigured && !this.allowDemoMode) {
      console.warn(
        "[Issuer DigiLocker] Credentials are not configured. Demo mode is disabled.",
      );
    }
  }
    ...
    return [
      {
        ...
        description: "University degree or diploma",
      },
      {
        code: "MARKSHEET",
        name: "Marksheet",
        description: "Academic grade card",
      },
      {
        code: "TRANSCRIPT",
        name: "Transcript",
      ...
      },
      {
        code: "AWARD",
        name: "Award",
        description: "Achievement or recognition award",
      },
      {
        code: "LICENSE",
...
function resolveTokenEncryptionKey(): string {
  ...
  return crypto
    .createHash("sha256")
    .update(
      process.env.DIGILOCKER_CLIENT_SECRET || "issuer-digilocker-user-pull-dev",
    )
    .digest("hex");
}
...
function mapListDocument(item: Record<string, any>): DigiLockerDocument {
  ...
  return {
    ...
    issuer: String(
      item.issuer || item.issuername || item.issuerid || "Unknown Issuer",
    ),
    date: String(item.date || item.issuedOn || item.issuanceDate || ""),
    mimeType: String(
      item.mimeType || item.mime || item.mimetype || "application/octet-stream",
    ),
...
async function persistUserPullState(): Promise<void> {
  ...
  userPullPersistChain = userPullPersistChain
    .then(() =>
      userPullStateStore.save({
        encryptedTokens: Array.from(userPullTokenStore.entries()),
      }),
    )
    .catch((error) => {
      console.error(
        "[Issuer DigiLocker] Failed to persist user-pull token state:",
        error,
      );
    });
 ...
 */
export async function getUserPullAuthUrl(
  userId: string,
  state: string,
): Promise<string> {
  if (
...
export async function exchangeUserPullCode(
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error(
      `Failed to exchange DigiLocker user-pull code: ${errorBody}`,
    );
  }
  ...
  if (!tokens.accessToken) {
    throw new Error(
      "DigiLocker user-pull token exchange returned no access token",
    );
  }
...
export async function pullDocument(
  ...
  const mimeType =
    ...
    (requestUrl.toLowerCase().endsWith(".pdf")
      ? "application/pdf"
      : "application/xml");
