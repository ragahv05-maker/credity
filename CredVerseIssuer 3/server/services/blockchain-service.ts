...
export class BlockchainService {
  ...
  constructor(config?: Partial<BlockchainConfig>) {
    ...
    if (!writePolicy.allowWrites) {
      console.warn(
        `[Blockchain] Writes disabled for ${this.chain}: ${writePolicy.reason}`,
      );
    }
    ...
    if (contractAddress && contractAddress.startsWith("0x")) {
      ...
      console.log(
        `[Blockchain] Attempting to configure with ${contractAddress} on ${this.chain} (${rpcUrl})`,
      );
      ...
      console.log(
        `[Blockchain] Running in deferred mode on ${this.chain} (no contract configuration)`,
      );
    }
  }
  ...
  private async validateContract(
    ...
    try {
      ...
      if (code === "0x" || code === "") {
        console.log(
          `[Blockchain] Contract NOT deployed at ${address} on ${this.chain} - deferred mode active`,
        );
        this.isConfigured = false;
      } else {
        console.log(
          `[Blockchain] Contract verified at ${address} on ${this.chain} (${rpcUrl})`,
        );
        this.isConfigured = true;
      }
    } catch (error: any) {
      console.log(
        `[Blockchain] RPC unreachable (${error.message}) on ${this.chain} - deferred mode active`,
      );
      this.isConfigured = false;
    }
  }
  ...
  async anchorCredential(credentialData: any): Promise<AnchorResult> {
    ...
    try {
      ...
      console.log(
        `[Blockchain] Anchored credential: ${hash} in tx ${receipt.hash} on ${this.chain}`,
      );
  ...
  async revokeCredential(
    ...
    try {
      ...
      console.log(
        `[Blockchain] Revoked credential: ${credentialHash} in tx ${receipt.hash} on ${this.chain}`,
      );
