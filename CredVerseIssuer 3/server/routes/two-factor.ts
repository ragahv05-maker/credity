...
router.post("/2fa/verify", async (req, res) => {
  try {
    ...
    if (!pendingToken) {
      return res
        .status(400)
        .json({ error: "Pending authentication token required" });
    }
    ...
    if (!pendingRecord) {
      return res
        .status(401)
        .json({ error: "Invalid pending authentication token" });
    }
    if (pendingRecord.expiresAt.getTime() < Date.now()) {
      pendingTwoFactorTokens.delete(pendingToken);
      return res
        .status(401)
        .json({ error: "Pending authentication token expired" });
    }
    if (String(pendingRecord.userId) !== normalizedUserId) {
      return res
        .status(401)
        .json({ error: "Pending authentication token mismatch" });
    }
  ...
  async (req, res) => {
    try {
      ...
      if (!token) {
        return res
          .status(400)
          .json({ error: "2FA token required to regenerate backup codes" });
      }
      ...
      res.json({
        ...
        backupCodes,
        warning:
          "Old backup codes are now invalid. Save these new codes in a secure place.",
      });
    } catch (error) {
