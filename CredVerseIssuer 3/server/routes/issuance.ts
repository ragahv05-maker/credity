...
router.post("/credentials/issue", writeIdempotency, async (req, res) => {
  try {
    ...
    const {
      ...
      normalizeRecipient,
      validateRecipient,
      validateCredentialDataAgainstTemplateSchema,
    } = await import("../services/issuance-validation");
    ...
    if (!template) {
      return res
        .status(404)
        .json({ message: "Template not found", code: "TEMPLATE_NOT_FOUND" });
    }
    const templateTenantId =
      typeof (template as any).tenantId === "string"
        ? String((template as any).tenantId).trim()
        : undefined;
    ...
    const schemaCheck = validateCredentialDataAgainstTemplateSchema(
    ...
    const status =
      message.includes("queue") || message.includes("REDIS_URL") ? 503 : 500;
    res
      .status(status)
      .json({
        message,
        code: status === 503 ? "QUEUE_UNAVAILABLE" : "ISSUANCE_FAILED",
      });
  }
});
...
router.get("/queue/stats", async (_req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to fetch queue stats" });
  }
});
...
router.get("/queue/jobs/:jobId", async (req, res) => {
  try {
    ...
    if (!status) {
      return res
        .status(404)
        .json({ message: "Job not found", code: "QUEUE_JOB_NOT_FOUND" });
    }
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to fetch queue job status" });
  }
});
...
router.get("/queue/dead-letter", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error.message || "Failed to fetch dead-letter queue" });
  }
});

router.post(
  "/queue/dead-letter/:entryId/replay",
  writeIdempotency,
  async (req, res) => {
    try {
      if (!authorizeQueueOperations(req as any, res)) {
...
router.post("/credentials/:id/revoke", writeIdempotency, async (req, res) => {
  try {
    ...
    const status = revocation.alreadyRevoked
      ? await getCredentialStatus(req.params.id)
      : await revokeCredentialStatus(req.params.id);

    res.json({
      message: revocation.alreadyRevoked
