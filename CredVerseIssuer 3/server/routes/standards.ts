...
router.post(
  "/api/v1/oid4vci/credential-offers",
    ...
    try {
      ...
      if (!templateId || !issuerId || !recipient || !credentialData) {
        return res
          ...
          .json({
            error:
              "templateId, issuerId, recipient and credentialData are required",
          });
      }
      ...
      res
        .status(500)
        .json({ error: error.message || "Failed to create credential offer" });
    }
  },
);
...
router.post("/api/v1/oid4vci/token", async (req, res) => {
  try {
    ...
    const { grant_type: grantType, "pre-authorized_code": preAuthorizedCode } =
      req.body || {};
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({ error: error.message || "Credential issuance failed" });
    }
  },
);
...
router.post("/api/v1/oid4vci/deferred", async (req, res) => {
  ...
  const configuredRetryAfter = Number(
    process.env.OID4VCI_DEFERRED_RETRY_AFTER_SECONDS || 5,
  );
  const retryAfter =
...
router.get("/api/v1/status/bitstring/:listId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error?.message || "Failed to resolve status list" });
  }
});
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const revocation = await issuanceService.revokeCredential(
        credentialId,
        reason,
      );
      const status = revocation.alreadyRevoked
        ? await getCredentialStatus(credentialId)
        : await revokeCredentialStatus(credentialId);
      ...
      res.json({
        ...
        status: status
          ...
          : null,
        code: revocation.alreadyRevoked
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to revoke credential" });
    }
  },
);

router.post(
  "/api/v1/anchors/batches",
  apiKeyOrAuthMiddleware,
  writeIdempotency,
  async (req, res) => {
    try {
      ...
      if (validRecords.length !== credentialIds.length) {
        return res
          .status(400)
          .json({ message: "One or more credentials do not exist" });
      }
      const tenantId = (req as any).tenantId;
      const hasForeignTenantRecord = validRecords.some(
      ...
      res
        .status(500)
        .json({ message: error.message || "Failed to create anchor batch" });
    }
  },
);
...
router.get("/api/v1/anchors/batches/:batchId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error?.message || "Failed to load anchor batch" });
  }
});

router.post(
  "/api/v1/anchors/batches/:batchId/replay",
  apiKeyOrAuthMiddleware,
  writeIdempotency,
  async (req, res) => {
    try {
  ...
  async (req, res) => {
    try {
      ...
      res
        .status(500)
        .json({
          message: error?.message || "Failed to load dead-letter entries",
        });
    }
  },
);
...
router.get("/api/v1/anchors/proofs/:credentialId", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: error?.message || "Failed to load anchor proof" });
  }
});
...
function parseProofFormat(
  format: unknown,
): ProofGenerationResultContract["format"] | null {
  if (typeof format !== "string") return null;
  return supportedProofFormats.has(
...
router.post(
  ...
  apiKeyOrAuthMiddleware,
  enforceProofRouteAccess,
  writeIdempotency,
  async (req, res) => {
    try {
      const payload = (req.body ||
        {}) as Partial<ProofGenerationRequestContract>;
      ...
      if (!credentialId && !payload.subject_did) {
        return res
          .status(400)
          .json({
            message: "credential_id or subject_did is required",
      ...
      if (credentialId) {
        ...
        if (credential.tenantId !== tenantId) {
          return res
            .status(403)
            .json({ message: "Forbidden", code: "PROOF_FORBIDDEN" });
        }
      }
      ...
      return res
        ...
        .json({
          ...
          code: "PROOF_GENERATE_INTERNAL_ERROR",
        });
    }
  },
);
...
router.get(
  "/api/v1/proofs/revocation-witness/:credentialId",
  apiKeyOrAuthMiddleware,
  enforceProofRouteAccess,
  async (req, res) => {
