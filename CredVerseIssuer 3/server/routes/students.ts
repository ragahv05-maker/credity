...
router.post("/students", async (req, res) => {
  try {
    const tenantId = (req as any).tenantId;
    const { name, email, studentId, program, enrollmentYear, status } =
      req.body;

    if (!name || !email || !studentId) {
      return res
        .status(400)
        .json({ message: "Name, email, and studentId are required" });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to create student", error: String(error) });
  }
});
