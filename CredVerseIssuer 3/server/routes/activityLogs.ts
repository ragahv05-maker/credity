  ...
  const actions = [
    {
      ...
      details: "Degree Certificate to Aditi Sharma",
      entityType: "credential" as const,
    },
    {
      action: "Updated template",
      details: "Modified Semester Grade Card layout",
      entityType: "template" as const,
    },
    {
      action: "Added student",
      details: "Imported 150 students via CSV",
      entityType: "student" as const,
    },
    {
      action: "Changed role",
      details: "Sarah Jenkins role changed to Issuer",
      entityType: "team" as const,
    },
    {
      action: "Revoked credential",
      details: "Revoked CRED-2023-001 due to error",
      entityType: "credential" as const,
    },
    {
      action: "Login",
      details: "Admin logged in from Mumbai, IN",
      entityType: "settings" as const,
    },
    {
      action: "Export",
      details: "Downloaded verification logs CSV",
      entityType: "settings" as const,
    },
    {
      action: "Invited member",
      details: "Sent invite to john@university.edu",
      entityType: "team" as const,
    },
  ];
...
router.post("/activity-logs", async (req, res) => {
  try {
    const tenantId = (req as any).tenantId;
    const { userId, userName, action, details, entityType, entityId } =
      req.body;
...
router.get("/reports/dashboard", async (req, res) => {
  try {
    ...
    // ⚡ Bolt Optimization: Parallelize independent database queries to reduce response time
...
${logs
  ...
  .map(
    (l) =>
      `${l.credentialId},${l.verifierName},${l.verifierLocation},${l.timestamp.toLocaleString()},${l.status}`,
  )
  .join("\n")}
`;
