...
async function resolveRevocationState(
  credentialId: string,
): Promise<"revoked" | "active" | "unknown"> {
  const credential = await storage.getCredential(credentialId);
  if (credential?.revoked) {
    return "revoked";
  }

  const credentialHashCandidate = (
    credential as unknown as { credentialHash?: unknown } | undefined
  )?.credentialHash;
  const onChainLookupKey =
    typeof credentialHashCandidate === "string" &&
    credentialHashCandidate.length > 0
...
async function getVerifierInfo(
  ip: string,
): Promise<{ location: string; organization: string }> {
...
router.get("/verify", async (req, res) => {
  try {
    ...
    try {
      ...
      return res
        .status(400)
        .json({ valid: false, message: "Invalid JWT format" });
    }
    ...
    console.log(
      `[VERIFY] Credential ${credentialId} verified by ${verifierInfo.organization} from ${verifierInfo.location}`,
    );
...
router.get("/verify/:credentialId", async (req, res) => {
  try {
    ...
    console.log(
      `[VERIFY] Credential ${credentialId} verified by ${verifierInfo.organization}`,
    );
